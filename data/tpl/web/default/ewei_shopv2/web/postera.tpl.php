			...
			<tr>
				...
				<td>
					...
					<?php if(cv('postera.edit|postera.view')) { ?>
						<a class='btn btn-default btn-sm' href="<?php  echo webUrl('postera/edit', array( 'id' => $row['id']))?>"><i class='fa fa-edit'></i> <?php if(cv('postera.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?></a>
					<?php  } ?>
					<?php  if($row['isdefault']==0) { ?>
					<?php if(cv('postera.setdefault')) { ?><a class='btn btn-default btn-sm' data-toggle='ajaxPost' href="<?php  echo webUrl('postera/setdefault', array('id' => $row['id']))?>" data-confirm="确认设置此海报为默认海报吗？"><i class='fa fa-check'></i> 默认</a><?php  } ?>
					<?php  } ?>
					<?php if(cv('postera.delete')) { ?><a class='btn btn-default btn-sm' data-toggle="ajaxRemove"  href="<?php  echo webUrl('postera/delete', array('id' => $row['id']))?>"  title='删除' data-confirm="确认删除此海报吗？"><i class='fa fa-trash'></i> 删除</a></td><?php  } ?>
