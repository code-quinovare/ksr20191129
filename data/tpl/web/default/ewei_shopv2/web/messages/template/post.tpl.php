...
<div class="page-heading"> 
	...
	<span class='pull-right'>
		...
		<a class="btn btn-default  btn-sm" href="<?php  echo webUrl('messages/template')?>">返回列表</a>
...
">
	...
	<h2><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>消息模板 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['title'];?>】<?php  } ?></small></h2> 
</div>
...
<div class="row">
	<div class="col-sm-8">
	 ...
	 <form <?php if( ce('messages.template' ,$list) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
                ...
                <div class="form-group miniprogramshow" <?php  if(empty($list['miniprogram'])) { ?>style="display: none;"<?php  } ?>>
                    <label class="col-sm-2 control-label" >小程序链接路径</label>
                    <div class="col-sm-9 col-xs-12">
                        <?php if( ce('messages.template' ,$list) ) { ?>
                        <div class="input-group form-group">
