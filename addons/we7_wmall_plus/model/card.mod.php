...
{
	global $_W;
	$order = pdo_get('tiny_wmall_plus_delivery_cards_order', array('uniacid' => $_W['uniacid'], 'id' => $order_id));
	if (empty($order)) {
		return error(-1, '订单不存在');
	}
	$pay_types = order_pay_types();
	$order['pay_type_cn'] = $pay_types[$order['pay_type']]['text'];
	$member = pdo_get('tiny_wmall_plus_members', array('uniacid' => $_W['uniacid'], 'uid' => $order['uid']));
	$card = pdo_get('tiny_wmall_plus_delivery_cards', array('uniacid' => $_W['uniacid'], 'id' => $order['card_id']));
	if (empty($card)) {
	...
	if (!empty($order['openid'])) {
		...
		$params = array('first' => $tips, 'OrderSn' => $order['ordersn'], 'OrderStatus' => '已生效', 'remark' => implode("\n", $remark));
		$url = $_W['siteroot'] . 'app' . ltrim(murl('entry', array('do' => 'card', 'm' => 'we7_wmall_plus')), '.');
		$send = sys_wechat_tpl_format($params);
...
{
	global $_W;
	$order = pdo_get('tiny_wmall_plus_delivery_cards_order', array('id' => $order_id, 'uniacid' => $_W['uniacid']));
	if (empty($order)) {
		return error(-1, '订单不存在');
	}
	if (!$order['is_pay']) {
		$card = pdo_get('tiny_wmall_plus_delivery_cards', array('id' => $order['card_id'], 'uniacid' => $_W['uniacid']));
		if (!empty($card)) {
			$member = pdo_get('tiny_wmall_plus_members', array('uid' => $order['uid'], 'uniacid' => $_W['uniacid']));
			if (!empty($member)) {
				$starttime = strtotime(date('Y-m-d'));
				if ($member['setmeal_endtime'] > $starttime) {
					$starttime = $member['setmeal_endtime'];
				}
				$endtime = strtotime("{$card['days']}days", $starttime);
				pdo_update('tiny_wmall_plus_delivery_cards_order', array('is_pay' => 1, 'paytime' => TIMESTAMP, 'starttime' => $starttime, 'endtime' => $endtime), array('id' => $order['id'], 'uniacid' => $_W['uniacid']));
				pdo_update('tiny_wmall_plus_members', array('setmeal_id' => $card['id'], 'setmeal_day_free_limit' => $card['day_free_limit'], 'setmeal_starttime' => $starttime, 'setmeal_endtime' => $endtime), array('uid' => $member['uid'], 'uniacid' => $_W['uniacid']));
