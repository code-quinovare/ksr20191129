...
{
	global $_W;
	$data = pdo_getall('tiny_wmall_plus_tables_category', array('uniacid' => $_W['uniacid'], 'sid' => $sid), array(), 'id');
	return $data;
}
function table_status()
{
	$data = array('0' => array(), '1' => array('css' => 'label label-default', 'css_block' => 'block-gray', 'text' => '空闲中'), '2' => array('css' => 'label label-danger', 'css_block' => 'block-red', 'text' => '已开台'), '3' => array('css' => 'label label-primary', 'css_block' => 'block-primary', 'text' => '已下单'), '4' => array('css' => 'label label-success', 'css_block' => 'block-success', 'text' => '已支付'));
	return $data;
}
function table_fetch($table_id)
{
	global $_W;
	$table = pdo_get('tiny_wmall_plus_tables', array('uniacid' => $_W['uniacid'], 'id' => $table_id));
	if (!empty($table)) {
		$table['category'] = pdo_get('tiny_wmall_plus_tables_category', array('uniacid' => $_W['uniacid'], 'id' => $table['cid']));
	}
	return $table;
}
...
{
	global $_W;
	$status = pdo_update('tiny_wmall_plus_tables', array('order_id' => $order_id, 'status' => $status), array('uniacid' => $_W['uniacid'], 'id' => $table_id));
	return $status;
}
function assign_board_status()
{
	$data = array('0' => array(), '1' => array('css' => 'label label-primary', 'text' => '排队中'), '2' => array('css' => 'label label-success', 'text' => '已入号'), '3' => array('css' => 'label label-warning', 'text' => '已过号'), '4' => array('css' => 'label label-danger', 'text' => '已取消'));
	return $data;
}
function assign_board_fetch($id)
{
	global $_W;
	$board = pdo_get('tiny_wmall_plus_assign_board', array('uniacid' => $_W['uniacid'], 'id' => $id));
	return $board;
}
function assign_queue_fetch($id)
{
	global $_W;
	$queue = pdo_get('tiny_wmall_plus_assign_queue', array('uniacid' => $_W['uniacid'], 'id' => $id));
	return $queue;
}
function assign_notice($sid, $id, $status)
{
	...
	if (!empty($config['notice']['assign_tpl'])) {
		...
		$url = murl('entry', array('m' => 'we7_wmall_plus', 'do' => 'assign', 'sid' => $sid, 'op' => 'goods'), true, true);
		$wait_count = pdo_fetchcolumn('select count(*) from ' . tablename('tiny_wmall_plus_assign_board') . ' where uniacid = :uniacid and sid = :sid and status = 1 and id < :id and queue_id = :queue_id', array(':uniacid' => $_W['uniacid'], ':sid' => $sid, ':queue_id' => $board['queue_id'], ':id' => $board['id']));
...
function assign_notice_queue($board_id, $queue_id)
{
	...
	if (!empty($queue) && $queue['notify_num'] > 0) {
		$boards = pdo_fetchall('select * from ' . tablename('tiny_wmall_plus_assign_board') . ' where uniacid = :uniacid and sid = :sid and queue_id = :queue_id and status = 1 and id > :id limit ' . $queue['notify_num'], array(':uniacid' => $_W['uniacid'], ':sid' => $queue['sid'], ':queue_id' => $queue_id, ':id' => $board_id));
		if (!empty($boards)) {
