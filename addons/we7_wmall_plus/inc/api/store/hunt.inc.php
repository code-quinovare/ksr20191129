...
if($op == 'search') {
	...
	if(!empty($goods)) {
		...
		$stores = pdo_fetchall('select id,title,logo,business_hours,delivery_price,send_price,delivery_time,delivery_mode from ' . tablename('tiny_wmall_plus_store') . " where uniacid = :uniacid and status = 1 and id in ({$sids_str})", array(':uniacid' => $_W['uniacid']), 'id');
	}
	$search_stores = pdo_fetchall('select id,title,logo,business_hours,delivery_price,send_price,delivery_time,delivery_mode,forward_mode,forward_url from ' . tablename('tiny_wmall_plus_store') . " where uniacid = :uniacid and status = 1 and id not in ({$sids_str}) and title like :key", array(':uniacid' => $_W['uniacid'], ':key' => "%{$key}%"));
