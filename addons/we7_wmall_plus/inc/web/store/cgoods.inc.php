...
if($op == 'post') {
	...
	if($id) {
		...
		if(empty($item)) {
			message('商品不存在或已删除', $this->createWebUrl('cgoods'), 'success');
		}
		if($item['is_options']) {
		...
		$item['attr']=iunserializer($item['attr']);
	} else {
		$item['total'] = -1;
		$item['unitname'] = '份';
	}
	if(empty($item['instruction'])) {
		...
		foreach($fields as $key => $val) {
			$instructions[] = "<div  style='font-size: 16px; color: #666; padding: 3px 0'>{$val}</div><ul class='list-paddingleft-2' style='list-style-type: disc;'><li style='color: #999; font-size: .7rem'></li></ul>";
		}
		$item['instruction'] = implode('', $instructions);
	}
	...
	if(checksubmit('submit')) {
		$data = array(
			...
			'instruction' => htmlspecialchars_decode($_GPC['instruction']),
		...
		$data['attr'] = iserializer(array(
			'tytitle' => array(
				'text' => '通用名称',
				'val' => trim($_GPC['attr']['tytitle']),
			),
			'jixing' => array(
				'text' => '剂型',
				'val' => trim($_GPC['attr']['jixing']),
			),
			'guige' => array(
				'text' => '规格',
				'val' => trim($_GPC['attr']['guige']),
			),
			'pzwh' => array(
				'text' => '批准文号',
				'val' => trim($_GPC['attr']['pzwh']),
			),
			'sccj' => array(
				'text' => '生产厂家',
				'val' => trim($_GPC['attr']['sccj']),
			),
		));
		...
		message('编辑商品成功', $this->createWebUrl('cgoods'), 'success');
	}
...
if($op == 'del') {
	...
	message('删除菜品成功', $this->createWebUrl('cgoods'), 'success');
}
...
if($op == 'export') {
	if(checksubmit()) {
		...
		if(is_error($file)) {
			message($file['message'], $this->createWebUrl('cgoods'), 'error');
		}
		$data = read_excel($file);
		if(is_error($data)) {
			message($data['message'], $this->createWebUrl('cgoods'), 'error');
		}
		unset($data[1]);
		if(empty($data)) {
			message('没有要导入的数据', $this->createWebUrl('cgoods'), 'error');
		}
		foreach($data as $da) {
		...
		message('导入商品成功', $this->createWebUrl('cgoods'), 'success');
	}
}
...
if($op == 'copy') {
	...
	message('复制商品成功, 现在进入编辑页', $this->createWebUrl('cgoods', array('op' => 'post', 'id' => $goods_id)), 'success');
}
include $this->template('store/cgoods');