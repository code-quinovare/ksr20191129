...
if($op == 'index') {
	...
	if($_W['isajax']) {
		...
		$stat['total_final_fee'] = floatval(pdo_fetchcolumn('select round(sum(final_fee), 2) from ' . tablename('tiny_wmall_plus_order') . $condition . ' and status = 5 and is_pay = 1', $params));
		...
		$stat['total_serve_fee'] = floatval(pdo_fetchcolumn('select round(sum(plateform_serve_fee), 2) from ' . tablename('tiny_wmall_plus_order') . $condition . ' and status = 5 and is_pay = 1', $params));
		$stat['total_delivery_fee'] = floatval(pdo_fetchcolumn('select round(sum(plateform_delivery_fee), 2) from ' . tablename('tiny_wmall_plus_order') . $condition . ' and status = 5 and is_pay = 1', $params));
		$stat['total_deliveryer_fee'] = floatval(pdo_fetchcolumn('select round(sum(plateform_deliveryer_fee), 2) from ' . tablename('tiny_wmall_plus_order') . $condition . ' and status = 5 and is_pay = 1', $params));
		$stat['total_refund_fee'] = floatval(pdo_fetchcolumn('select round(sum(refund_fee), 2) from ' . tablename('tiny_wmall_plus_order') . $condition . ' and refund_status > 1', $params));

		$chart = array(
			'stat' => $stat,
			'fields' => array('total_success_order', 'total_fee', 'final_fee', 'plateform_delivery_fee', 'plateform_serve_fee', 'plateform_deliveryer_fee', 'refund_fee'),
			'titles' => array('有效订单量','营业总额','总入账','平台配送费收入','佣金收入','配送员配送费支出','总退款'),
		);
	...
	$records_temp = pdo_fetchall('SELECT stat_day, count(*) as total_success_order, round(sum(total_fee), 2) as total_fee, round(sum(final_fee), 2) as final_fee, round(sum(plateform_serve_fee), 2) as plateform_serve_fee, round(sum(plateform_delivery_fee), 2) as plateform_delivery_fee,
	...
	$cancel_records = pdo_fetchall('SELECT stat_day, round(sum(refund_fee), 2) as refund_fee
	 FROM ' . tablename('tiny_wmall_plus_order') . $condition . ' and refund_status > 1 group by stat_day', $params, 'stat_day');
	$records = array();
	...
	include $this->template('plateform/analyse-takeout');
}
