...
$tables = array(
	'order' => 'tiny_wmall_plus_order',
	'card' => 'tiny_wmall_plus_delivery_cards_order',
	'errander' => 'tiny_wmall_plus_errander_order'
);
...
$record = pdo_get('tiny_wmall_plus_paylog', array('uniacid' => $_W['uniacid'], 'order_id' => $id, 'order_type' => $type));
if(empty($record)) {
	$record = array(
		'uniacid' => $_W['uniacid'],
	...
	pdo_insert('tiny_wmall_plus_paylog', $record);
} else {
	if($record['status'] == 1) {
...
$available_pay_types = $_W['we7_wmall_plus']['config']['payment'];
