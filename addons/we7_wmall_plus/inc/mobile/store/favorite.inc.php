...
$title = '我收藏的门店';
if($op == 'list') {
	$stores = pdo_fetchall('select a.id as aid, b.* from ' . tablename('tiny_wmall_plus_store_favorite') . ' as a left join ' . tablename('tiny_wmall_plus_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.uid = :uid order by a.id desc', array(':uniacid' => $_W['uniacid'], ':uid' => $_W['member']['uid']), 'aid');
	$min = 0;
	if(!empty($stores)) {
