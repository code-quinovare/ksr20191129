...
define('we7_wmall_plus_ROOT', dirname(dirname(__FILE__)));
define('IS_OPERATOR', true);
require we7_wmall_plus_ROOT . '/admin/common/bootstrap.sys.inc.php';
require we7_wmall_plus_ROOT . '/admin/common/template.func.php';
require we7_wmall_plus_ROOT . '/admin/common/common.func.php';
...
$controllers = array();
$handle = opendir(we7_wmall_plus_ROOT . '/admin/source/');
if(!empty($handle)) {
...
$init = we7_wmall_plus_ROOT . "/admin/source/{$controller}/__init.php";
if(is_file($init)) {
	require $init;
}
$actions = array();
$handle = opendir(we7_wmall_plus_ROOT . '/admin/source/' . $controller);
if(!empty($handle)) {
	while($dir = readdir($handle)) {
...
function _forward($c, $a) {
	$file = we7_wmall_plus_ROOT . '/admin/source/' . $c . '/' . $a . '.ctrl.php';
	return $file;
}