...
from_user=:from_user AND optionid=:optionid ", array(':goodsid' => $dishid, ':weid' => $weid, ':storeid' => $storeid, ':from_user' => $from_user, ':optionid' => $optionid));
        ...
        foreach ($cart as $key => $value) {
            ...
            if ($value['total'] > 0) {
                ...
                $cart_html .= '<tr dishid="' . $value['goodsid'] . '" optionid="' . $value['optionid'] . '"><td>' . $goods_t['title'] . $optionname . '</td><td>' . $value['total'] . '</td><td>yen<font>' . $value['price'] . '</font></td><td><i class="i-add-btn">+</i> <i class="i-remove-btn">-</i></td></tr>';
            }
        }
        $cart_html .= '';
