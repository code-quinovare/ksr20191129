<?php
		...
		if ($operation == 'display') {
			...
			if (empty($category)) {
				itoast('抱歉，分类不存在或是已经被删除！', $this->createWebUrl('category', array('op' => 'display')), 'error');
			}
			if (pdo_delete($this->tb_category, array('id' => $id)) === false) {
				itoast('删除分类失败, 请稍后重试.');
				exit();
			}
//			pdo_delete('cover_reply', array('rid' => $category['rid']));
//			pdo_delete('rule_keyword', array('rid' => $category['rid']));
//			pdo_delete('rule', array('id' => $category['rid']));			
			itoast('分类删除成功！', $this->createWebUrl('category', array('op' => 'display')), 'success');
		}
