...
class Cy163_customerserviceModuleSite extends WeModuleSite {
	...
	public function doMobileGetquickgroup(){
		global $_GPC, $_W;
		$renrengoodsid = intval($_GPC['renrengoodsid']);
		$qudao = trim($_GPC['qudao']);
		if($renrengoodsid > 0 && $qudao == 'renren' && pdo_tableexists('ewei_shop_goods')){
			$goodsres = pdo_fetch("SELECT shopid,merchid FROM ".tablename('ewei_shop_goods')." WHERE id = {$renrengoodsid} AND uniacid = {$_W['uniacid']}");
			if($goodsres['merchid'] > 0){
				$cservicegroup = pdo_fetch("SELECT * FROM ".tablename(BEST_CSERVICEGROUP)." WHERE sanbs = '{$qudao}' AND weid = {$_W['uniacid']} AND bsid = {$goodsres['merchid']}");
			}else{
				$cservicegroup = pdo_fetch("SELECT * FROM ".tablename(BEST_CSERVICEGROUP)." WHERE sanbs = '{$qudao}' AND weid = {$_W['uniacid']}");
	...
	public function addxcxchat($postArr){
		...
		$cservice = pdo_fetch("SELECT * FROM ".tablename(BEST_XCXCSERVICE)." WHERE weid = {$_W['uniacid']} AND xcxid = {$xcx['id']}");
		...
		if(!empty($has)){
			if($postArr['MsgType'] == 'text'){
				...
				$dataup['lastcon'] = $postArr['Content'];
			}
			...
			$this->xcxtzkefu($kefuopenid,$has['lasttime'],$has['id'],$dataup['lastcon'],$xcx['name']);
			
			//include $this->template('newxcxchat');
			
			/*$socket = socket_create(AF_INET, SOCK_STREAM, SOL_TCP);
			
			$socket_result = socket_connect($socket, "https://api.qiumipai.com", "2120");
			
			$myfile = fopen("messi.txt", "w") or die("Unable to open file!");
			fwrite($myfile, $socket);
			fclose($myfile);
			
			$post_data2 = array();
			$kkk = $this->messi_request_post("https://api.qiumipai.com:2120", $post_data2);
			
			$post_url = 'https://api.qiumipai.com:2121/?type=xcxpublish';
			$post_data['to'] = $has['kefuopenid'];
			$post_data['content'] = $data['content'];
			$post_data['msgtype'] = $data['msgtype'];
			$post_data['toopenid'] = $has['fansopenid'];

			$kk = $this->messi_request_post($post_url, $post_data);
	...
	public function xcxtuisong(){
		global $_W,$_GPC;
	}
	
	
	public function messi_request_post($url = '', $post_data = array()) {
		...
		foreach($chatcon as $k=>$v){
			...
			if($v['openid'] != $hasfanskefu['kefuopenid']){
				...
				$html .= $htmltime.'<div class="left flex">
										<img src="../addons/cy163_customerservice/static/xcx.png" class="avatar" />
	...
	public function doMobileXcxchat(){
		...
		pdo_update(BEST_XCXFANSKEFU,array('notread'=>0,'huifunum'=>0),array('id'=>$hasfanskefu['id']));
	...
	public function doMobileAddchatxcx(){
		...
		if($hasfanskefu['huifunum'] >= 5){
			$resArr['error'] = 1;
			$resArr['msg'] = '您发送了5条消息，对方未回应，暂时不能发送啦！';
			echo json_encode($resArr);
			exit;
		}
		...
		$this->addxcxchat2($fanskefu['fansopenid'],$chatcontent,$fanskefu['gh_id']);
		
		pdo_update(BEST_XCXFANSKEFU,array('huifunum'=>$fanskefu['huifunum']+1),array('id'=>$fanskefu['id']));
        $resArr['error'] = 0;
        $resArr['msg'] = '';
	...
	public function xcxtzkefu($openid,$lasttime,$fkid,$content,$xcxname){
		global $_GPC, $_W;
	...
	public function addxcxchat2($touser,$content,$gh_id){
		global $_GPC, $_W;
	...
	public function addxcxfanskefu($postArr){
		global $_GPC, $_W;
		$xcx = pdo_fetch("SELECT * FROM ".tablename(BEST_XCX)." WHERE uniacid = {$_W['uniacid']} AND gh_id = '{$postArr['ToUserName']}'");
		$cservice = pdo_fetch("SELECT * FROM ".tablename(BEST_XCXCSERVICE)." WHERE weid = {$_W['uniacid']} AND xcxid = {$xcx['id']} ORDER BY rand() LIMIT 1");
		$kefuopenid = $cservice['content'];
		$gh_id = $postArr['ToUserName'];
		$fansopenid = $postArr['FromUserName'];
		$has = pdo_fetch("SELECT * FROM ".tablename(BEST_XCXFANSKEFU)." WHERE weid = {$_W['uniacid']} AND kefuopenid = '{$kefuopenid}' AND fansopenid = '{$fansopenid}' AND gh_id = '{$gh_id}'");
	...
	public function curl_post($url,$data=array(),$timeout=20){		
		//发送curl，返回arr。
        $ch = curl_init();
		...
		if ($operation == 'display') {
			...
			if(!empty($kefuopenid)){				
				...
				foreach($fanslist as $k=>$v){
					$fanslist[$k]['chat'] = pdo_fetchall("SELECT * FROM ".tablename(BEST_CHAT)." WHERE fkid = {$v['id']} ORDER BY time DESC");
				}
	...
	public function doWebCservice() {
		...
		if ($operation == 'display') {
			...
			if($_GPC['groupid']){
				...
				$kefuids .= "0";
				$kefuids .= ")";
				$cservicelist = pdo_fetchall("SELECT * FROM " . tablename(BEST_CSERVICE) . " WHERE (groupid = '{$_GPC['groupid']}' OR id in {$kefuids}) ORDER BY displayorder ASC");
			}else{
			...
			foreach($cservicelist as $k=>$v){
				if($v['ctype'] == 1){
					...
					$cservicelist[$k]['pingjia'] = pdo_fetchall("SELECT * FROM ".tablename(BEST_PINGJIA)." WHERE kefuopenid = '{$v['content']}' ORDER BY time DESC");
				}
				if($v['ctype'] == 2){
	...
	public function doWebCservicegroup() {
		...
		if ($operation == 'display') {
			...
			foreach($cservicegrouplist as $k=>$v){
				...
				$scripturl = $_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry'.'&do=getquickgroup&m=cy163_customerservice&qudao='.$v['sanbs'];
				$scripthtml = '<script type="text/javascript" src="../addons/cy163_customerservice/static/js/jquery-3.1.1.min.js"></script>
				...
				$cservicegrouplist[$k]['texturl'] = $_W['siteroot'].'app/index.php?i='.$_W['uniacid'].'&c=entry&id='.$v['id'].'&do=getquickgroupym&m=cy163_customerservice&qudao='.$v['sanbs'];
			}
			...
			if (checksubmit('submit')) {
				...
				$data = array(
					...
					'thumb' => $_GPC['thumb'],
					'displayorder' => intval($_GPC['displayorder']),
					'qrtext'=>trim($_GPC['qrtext']),
					'qrcolor'=>trim($_GPC['qrcolor']),
					'qrbg'=>trim($_GPC['qrbg']),
					'ishow' => intval($_GPC['ishow']),
					'sanbs'=>trim($_GPC['sanbs']),
					'sanremark'=>trim($_GPC['sanremark']),
					'bsid' => intval($_GPC['bsid']),
					'qrright' => intval($_GPC['qrright']),
					'qrbottom' => intval($_GPC['qrbottom']),
					'fid' => intval($_GPC['fid']),
				);
				if (!empty($id)) {
	...
	public function doMobileChosekefu() {
		...
		$iscservice = pdo_fetch("SELECT id FROM ".tablename(BEST_CSERVICE)." WHERE weid = {$_W['uniacid']} AND ctype = 1 AND content = '{$_W['fans']['from_user']}'");
		if(!empty($iscservice)){
			$notread = pdo_fetchcolumn("SELECT SUM(`notread`) FROM ".tablename(BEST_FANSKEFU)." WHERE kefuopenid = '{$_W['fans']['from_user']}'");
		}else{
			$notread = pdo_fetchcolumn("SELECT SUM(`kefunotread`) FROM ".tablename(BEST_FANSKEFU)." WHERE weid = {$_W['uniacid']} AND fansopenid = '{$_W['fans']['from_user']}'");
		}
	...
	public function doMobileGroupchat() {
		...
		$cservicegrouplist = pdo_fetchall("SELECT * FROM ".tablename(BEST_CSERVICEGROUP)." WHERE weid = {$_W['uniacid']} AND ishow = 1 AND fid = {$id} ORDER BY displayorder ASC");
	...
	public function doMobileChat(){
		...
		if($this->module['config']['bdmodel'] == 1){
			...
			if(!empty($hasbd)){
				if($hasbd['kefuopenid'] != $toopenid){
					$cservice = pdo_fetch("SELECT * FROM ".tablename(BEST_CSERVICE)." WHERE weid = {$_W['uniacid']} AND ctype = 1 AND content = '{$hasbd['kefuopenid']}'");
					if(empty($cservice)){
						$message = '获取客服信息失败！';
						include $this->template('error');
						exit;
		...
		$cjwtlist = pdo_fetchall("SELECT a.* FROM ".tablename(BEST_WENZHANG)." as a,".tablename(BEST_KEFUANDCJWT)." as b WHERE a.weid = {$_W['uniacid']} AND b.kefuid = {$cservice['id']} AND a.id = b.wtid");
	...
	public function doMobileAllshare(){
		...
		if($this->module['config']['sharetype'] == 1 && $cservice['groupid'] == 0){
...
// 本vv模vv块vv来vvv自vv新vv睿v社vvv区！	
	public function doMobileAddchat(){
		...
		if($this->module['config']['zdhf'] == 1){
			$zdhf = pdo_fetch("select * from ".tablename(BEST_ZIDONGHUIFU)." where weid = {$_W['uniacid']} AND title = '{$chatcontent}' AND type = 1");
			if(!empty($zdhf)){
				$resArr['jqr'] = 1;
				$jqrtime = $data['time']+1;
				...
				$zdhf2 = pdo_fetch("select * from ".tablename(BEST_ZIDONGHUIFU)." where weid = {$_W['uniacid']} AND title like '%{$chatcontent}%' AND type = 2");
				if(!empty($zdhf2)){
					$resArr['jqr'] = 1;
					$jqrtime = $data['time']+1;
					$resArr['jqrtime'] = date("Y-m-d H:i:s",$jqrtime);
					$resArr['jqrcontent'] = $zdhf2['content'];
					$resArr['jqravatar'] = tomedia($cservice['thumb']);
	...
	public function doMobileMychat(){
		...
		if ($operation == 'display') {
			...
			$psize = 20;
			if(!empty($isservice)){
				$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename(BEST_FANSKEFU)." WHERE kefuopenid = '{$openid}' AND lastcon != '' AND kefudel = 0");
				$allpage = ceil($total/$psize)+1;
				$page = intval($_GPC["page"]);
				$pindex = max(1, $page);
				
				$chatlist = pdo_fetchall("SELECT * FROM ".tablename(BEST_FANSKEFU)." WHERE kefuopenid = '{$openid}' AND lastcon != '' AND kefudel = 0 ORDER BY notread DESC,lasttime DESC LIMIT ".($pindex - 1)*$psize.",".$psize);
			}else{
