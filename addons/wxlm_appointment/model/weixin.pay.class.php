...
class WeiXinPay
{
	...
	{
		$params = array('appid' => $this->wxpay['appid'], 'mch_id' => $this->wxpay['mch_id'], 'time_stamp' => TIMESTAMP, 'nonce_str' => random(32), 'product_id' => $product_id);
		$params['sign'] = $this->bulidSign($params);
	...
	public function NoticeMicroSuccessOrder($result)
	{
		...
		$data = array('status' => 1, 'openid' => $result['openid']);
	...
	public function buildJsApiPrepayid($product_id)
	{
		...
		$jspai = array('out_trade_no' => $order['uniontid'], 'trade_type' => 'JSAPI', 'openid' => $order['openid'], 'body' => $order['tid'], 'total_fee' => $order['fee'] * 100, 'attach' => $order['uniacid']);
		$result = $this->buildUnifiedOrder($jspai);
		if (is_error($result)) {
			return $result;
		}
		$jspai = array('appId' => $this->wxpay['appid'], 'timeStamp' => TIMESTAMP, 'nonceStr' => random(32), 'package' => 'prepay_id=' . $result['prepay_id'], 'signType' => 'MD5');
	...
	public function buildNativePrepayid($product_id)
	{
		...
		$data = array('body' => $order['body'], 'out_trade_no' => $order['uniontid'], 'total_fee' => $order['fee'] * 100, 'trade_type' => 'NATIVE', 'product_id' => $order['plid'], 'attach' => $order['uniacid']);
		$result = $this->buildUnifiedOrder($data);
		if (is_error($result)) {
			return $result;
		}
		$params = array('return_code' => 'SUCCESS', 'appid' => $this->wxpay['appid'], 'mch_id' => $this->wxpay['mch_id'], 'prepay_id' => $result['prepay_id'], 'nonce_str' => random(32), 'result_code' => 'SUCCESS', 'code_url' => $result['code_url']);
		$params['sign'] = $this->bulidSign($params);
		return $params;
	}
	...
	{
		$params = array('appid' => $this->wxpay['appid'], 'mch_id' => $this->wxpay['mch_id'], 'nonce_str' => random(32), 'out_trade_no' => trim($trade_no));
		$params['sign'] = $this->bulidSign($params);
	...
	{
		$params = array('appid' => $this->wxpay['appid'], 'mch_id' => $this->wxpay['mch_id'], 'nonce_str' => random(32));
		if ($type == 1) {
	...
	{
		$params = array('appid' => $this->wxpay['appid'], 'mch_id' => $this->wxpay['mch_id'], 'nonce_str' => random(32), 'bill_date' => $date, 'bill_type' => $type);
		$params['sign'] = $this->bulidSign($params);
