...
require IA_ROOT.'/addons/dayu_form/inc/func/core.php';
class dayu_formModuleSite extends Core{
...
//			$oauth2_code = $this->createMobileUrl('list', array(), true);
	...
	public function __web($f_name){
		...
		include_once  MODULE_ROOT.'/inc/web/'.strtolower(substr($f_name,5)).'.php';
	}
	...
	public function __mobile($f_name){
		...
		include_once MODULE_ROOT.'/inc/mobile/'.strtolower(substr($f_name, 8)).'.php';
	}
	...
	public function doWebLinkage() {
		...
		if ($operation == 'display') {
			...
			$linkage = pdo_fetch("SELECT * FROM ".tablename($this->tb_linkage)." WHERE id = '$id'");
			if (empty($linkage)) {
				itoast('抱歉，联动不存在或是已经被删除！', referer(), 'error');
			}
			pdo_delete($this->tb_linkage, array('id' => $id));
			itoast('联动删除成功！', referer(), 'success');
		}
	}
		...
		if($op == 'list') {
			...
			if(checksubmit('submit')) {
				if(!empty($_GPC['ids'])) {
					...
					itoast('编辑成功', $this->createWebUrl('staff', array('op' => 'list', 'reid' => $reid)), 'success');
				}
			}
			...
			if(checksubmit('submit')) {
				...
				itoast('添加客服成功', $this->createWebUrl('staff', array('reid' => $reid, 'op' => 'list')), 'success');
			}
			include $this->template('staff');
		} elseif ($op == 'staffdel') {
			$id = intval($_GPC['id']);
	...
	public function doWebchangecheckedAjax(){
		...
		if ($setting['role']==1 && $_W['role'] == 'operator' && !$role) message('您没有权限进行该操作.');
//		$data = array('kfid' => $kfid);
//		$filter  = array('reid' => intval($id), 'weid' => $_W['uniacid']);
			if(false !== pdo_update($this->tb_form, array($field => $change), array('reid' => intval($id), 'weid' => $_W['uniacid']))){
				exit('1');
			}else{
				exit('0');
			}
    ...
    public function doWebDetail() {
		...
		$thumb1 = unserialize($row['rethumb']);
		$rethumb = array();
		if(is_array($thumb1)){
			foreach($thumb1 as $p){
				$rethumb[] = is_array($p)?$p['attachment']:$p;
			}
		}
		$la = iunserializer($activity['linkage']);
        ...
        foreach ($ds as $value) {
			...
			if (is_array($_GPC['rethumb'])) {
				foreach ($_GPC['rethumb'] as $thumb) {
					$th[] = tomedia($thumb);
				}
			...
			if (!empty($_FILES['upfile']['tmp_name'])) {
				$upload = file_upload($_FILES['upfile'], 'image');
				if (is_error($upload)) {
					die('<script>alert("上传失败！请重试！");location.reload();</script>');
				}
				$record['file'] = $upload['path'];
			}
			**/
...
//				$status = $account_api->sendCustomNotice(urldecode(json_encode($custom)));
			...
			if($activity['custom_status'] == '0' && !empty($activity['m_templateid'])){
				$data = array (
					'first' => array('value' => $activity['mfirst']."\n",'color'=>"#743A3A"),
					'keyword1' => array('value' => $row['member']),
					'keyword2' => array('value' => $row['mobile']),
					'keyword3' => array('value' => $_GPC['yuyuetime']),
					'keyword4' => array('value' => $huifu),
					'remark' => array('value' => "\n".$activity['mfoot'],'color'=>"#008000")
				);
				$acc = WeAccount::create($_W['acid']);
				$acc->sendTplNotice($row['openid'],$activity['m_templateid'],$data,$outurl,"#FF0000");
			}
		...
		load()->func('file');
        load()->func('tpl');
		$_W['page']['title'] = $activity['title'].' 表单详情';
    ...
    public function doWebManage() {
		...
		*/
        ...
        if (!empty($total)) {
			...
			$lists = pdo_fetchall("SELECT * FROM ".tablename($this->tb_info)." WHERE $where", $params, 'rerid');
            load()->model('mc');
            foreach ($list as &$r) {
	...
	//				print_r($consult);
		...
		*/
        ...
        if (!empty($_GPC['export'])) {
			...
			$htmlheader = array('openid'=>'粉丝编号', 'member'=>$activity['member'], 'mobile'=>$activity['phone']);
			...
			$zuhe=array_merge($htmlheader,$ds,$linkage,$pluraltit,$var1t,$var2t,$var3t,$htmlfoot);
					
	//		$html = $this->from_export_parse($zuhe,$select,$childrens,$reid,$rerid);
...
//				$linkage = pdo_fetchall('SELECT * FROM ' . tablename($this->tb_linkage) . ' WHERE reid = :reid', array(':reid' => $id));
//				if(!empty($linkage)) {
//					foreach($linkage as &$la) {
//						unset($la['id']);
//						$la['reid'] = $form_id;
//						pdo_insert($this->tb_linkage, $la);
//					}
//				}
  ...
  //          $item['follow'] = $item['par']['follow']==1 ? '<span class="btn btn-success btn-sm">启用</span>' : '<span class="btn btn-default btn-sm">关闭</span>';
  //          $item['isvoice'] = $item['isvoice']==1 ? '<span class="btn btn-success btn-sm">启用</span>' : '<span class="btn btn-default btn-sm">关闭</span>';
  //          $item['isrevoice'] = $item['isrevoice']==1 ? '<span class="btn btn-success btn-sm">启用</span>' : '<span class="btn btn-default btn-sm">关闭</span>';
 //           $item['header'] = $item['par']['header']==1 ? '<span class="btn btn-success btn-sm">启用</span>' : '<span class="btn btn-default btn-sm">关闭</span>';
    ...
    public function doWebDelete() {
        ...
        $reid = intval($_GPC['id']);
		$role=$this->get_isrole($reid,$_W['user']['uid']);
		if ($_W['role'] == 'operator' && !$role) message('您没有权限进行该操作.');
        if ($reid > 0) {
            $params = array();
            $params[':reid'] = $reid;
    ...
    public function doWebPost() {
        ...
        if (checksubmit()) {
            $data = array(
                ...
                'isdel' => intval($_GPC['isdel']),
                'follow' => intval($_GPC['follow']),
                'replace' => intval($_GPC['replace']),
				'card' => intval($_GPC['card']),
                ...
                'var1t' => trim($_GPC['var1t']),
                'var1' => trim($_GPC['var1']),
                'var2t' => trim($_GPC['var2t']),
                'var2' => trim($_GPC['var2']),
                'var3t' => trim($_GPC['var3t']),
                'var3' => trim($_GPC['var3']),
                'title' => trim($_GPC['titles']),
                'mname' => trim($_GPC['mname']),
                'submitname' => trim($_GPC['submitname']),
                ...
                'noticeurl' => trim($_GPC['noticeurl']),
                'kami' => intval($_GPC['kami']),
                'print' => intval($_GPC['print']),
                'sendkami' => intval($_GPC['sendkami']),
                'comment' => intval($_GPC['comment']),
                'commenttitle' => trim($_GPC['commenttitle']),
                'consult' => intval($_GPC['consult']),
                'getadd' => intval($_GPC['getadd']),
                'icredit' => intval($_GPC['icredit']),
                'onlytit' => trim($_GPC['onlytit']),
                'sms' => $_GPC['sms'],
                'smstype' => $_GPC['smstype'],
                'subtitle' => trim($_GPC['subtitle']),
                'icon' => $_GPC['icon'],
                'isrand' => $_GPC['isrand'],
                'randnum' => $_GPC['randnum'],
            );
            $record = array();
...
//					$field['only'] = $_GPC['onlyvalue'][$k] == 'true' ? 1 : 0;
        ...
        if ($reid) {
			...
			$par = iunserializer($activity['par']);
			!empty($activity['slide']) && $slide = iunserializer($activity['slide']);
        ...
        if (!$reply) {
            ...
            $activity = array(
                "kfirst" => "有客户提交新的表单，请及时跟进",
                "kfoot" => "点击处理客户提交的表单。",
                "mfirst" => "受理结果通知",
                "mfoot" => "如有疑问，请致电联系我们。",
                "information" => "您提交申请我们已经收到, 请等待客服跟进.",
                "adds" => "联系地址",
                "voice" => "录音",
                "voicedec" => "录音说明",
                "pluraltit" => "上传图片",
...
//		$title=$activity['title'];
	...
	public function doMobileCheckOnly(){
		global $_W, $_GPC;
//		$boss = pdo_get($this->boss_table, array('weid' => $_W['uniacid'], 'id' => $_GPC['bid']), array());
...
//		checkAuth();
		...
		if ($par['edit'] != '1' && $info['status'] !='8' ) $this->showMessage('不能修改内容','error');
        $ds = $fids = array();
        foreach ($field as $f) {
            if ($f['type'] == 'reside') {
				$reside = $f;
	...
	//			$datas[] = $entry;
			...
			$row['status'] = '0';
			pdo_update($this->tb_info, $row, array('rerid' => $rerid));
            ...
            if (!empty($datas)) {
				...
				/**
				if (!empty($activity['mobile']) && $activity['smsnotice']!=0) {
					load()->func('communication');
					$content = $activity['smstype']==1 ? $activity['title'] : $smsbody;
					ihttp_post(murl('entry', array('do' => 'Notice', 'id' => $activity['smsnotice'], 'm' => 'dayu_sms'), true, true), array('mobile' => $activity['mobile'], 'mname' => $info['member'], 'mmobile' => $info['mobile'], 'content' => $content));
				}
				**/
				$ytime = date('Y-m-d H:i:s', TIMESTAMP);
					...
					if(is_array($staff)) {
						foreach($staff as $s) {
							$template =array(
								...
								"topcolor"=>"#FF0000",
								"data"=>array('first'=>array('value'=>urlencode($activity['kfirst']."\\n"),'color'=>"#743A3A",),
											'keyword1'=>array('value'=>urlencode($info['member']),'color'=>'#000000',),
											'keyword2'=>array('value'=>urlencode($info['mobile']),'color'=>'#000000',),
											'keyword3'=>array('value'=>urlencode($ytime),'color'=>'#000000',),
											'keyword4'=>array('value'=>urlencode($status['name'].'\\n'.$bodnew."\\n"),'color'=>"#FF0000",),
											'remark'=>array('value'=>urlencode($activity['kfoot']),'color'=>"#008000",),
								)
							);
//							load()->func('communication');
...
//		$this->checkAuth();
//		print_r($_W['current_module']['name']);
		$returnUrl = urlencode($_W['siteurl']);
        $reid = intval($_GPC['id']);
        $sql = 'SELECT * FROM ' . tablename($this->tb_form) . ' WHERE `weid`=:weid AND `reid`=:reid';
        $params = array();
        ...
        $activity = pdo_fetch($sql, $params);
		$par = iunserializer($activity['par']);
		!empty($activity['linkage']) && $la = iunserializer($activity['linkage']);
		!empty($activity['slide']) && $slide = iunserializer($activity['slide']);
        if ($par['follow'] == 1) {
			$this->getFollow();
        }
...
//		$mcredit=$credits[$behavior['activity']]*100;
//		$acredit=$activity['credit']*100;
...
//		$title = $activity['title'];
		...
		if($activity['mbgroup']!=0){
			if($groupid != $activity['mbgroup']){$this->showMessage('您所在会员组没有相关的操作权限！', '','info');}
		}
        if ($par['isrand'] == 1 && !empty($par['randnum'])) {
			$sql = 'SELECT * FROM ' . tablename($this->tb_field) . ' WHERE `reid` = :reid ORDER BY rand() limit '.$par['randnum'];
        }else{
		...
		if ($par['card']==1 || $par['card']==2) {
			...
			$to_card = $par['card']==1 ? murl('entry', array('do' => 'card', 'm' => 'we7_coupon', 'returnurl' => $returnUrl), true, true) : murl('entry', array('do' => 'index', 'm' => 'dayu_card', 'returnurl' => $returnUrl), true, true);
			if($ishy==false){
...
//		if ($activity['smsid']!=0 && $activity['paixu']!='2') {
//			$ismobile = pdo_get('dayu_sms_log', array('weid' =>$weid, 'mobile' => $member['mobile']), array('id'));
//		}		
		($activity['smsid'] && empty($member['mobile']) && $par['smstype']=='1') && $this->showMessage('请完善手机号', murl('entry', array('do' => 'index', 'id' => $activity['smsid'], 'm' => 'dayu_sms', 'form' => $_W['current_module']['name'], 'returnurl' => $returnUrl), true, true), 'info');
		
		$mname = !empty($par['mname']) ? $par['mname'] : "往期记录";
		$submitname = !empty($par['submitname']) ? $par['submitname'] : '立 即 提 交';
		$btncolor = $this->get_color($reid,$par['btncolor']);
		$btncolor = $btncolor['css'];
//		if($activity['isloc']==1){
//			$fansloc = pdo_fetch("SELECT * FROM ".tablename($this->loc_table)." WHERE weid = :weid AND openid = :openid", array(':weid' => $weid, ':openid' => $openid));
//			if($_GPC['op'] == "locate"){
//				if($fansloc['loc_x']){
//					pdo_update($this->loc_table, array('loc_x' => $_GPC['loc_x'], 'loc_y' => $_GPC['loc_y'], 'createtime' =>TIMESTAMP), array('openid' => $openid, 'weid' => $weid));
//				}else{
//					pdo_insert($this->loc_table, array('loc_x' => $_GPC['loc_x'], 'loc_y' => $_GPC['loc_y'], 'createtime' =>TIMESTAMP,'openid' => $openid, 'weid' => $weid));
//				}
//			}
//		}
...
//		$pindex = max(1, intval($_GPC['page']));
//		$psize = 10;
//		$sql = 'SELECT * FROM ' . tablename($this->tb_info) . " WHERE reid=:reid and ( status=1 or status=3 ) ORDER BY createtime DESC,rerid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize;
//		$params = array();
//		$params[':reid'] = $reid;
//		$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE reid = :reid and ( status=1 or status=3 ) ", $params);
//		$pager = $this->pagination($total, $pindex, $psize);
//		$record = pdo_fetchall($sql, $params);
//		foreach ($record as $index => $row) {
//			$record[$index]['user'] = mc_fansinfo($row['openid'],$acid, $weid);
//		}
//		$linkage = pdo_fetchall("SELECT * FROM ".tablename($this->tb_linkage)." WHERE reid = :reid and parentid = 0 ORDER BY id DESC, displayorder DESC", array(':reid' => $reid));
        ...
        foreach ($ds as &$r) {
			...
			}elseif (in_array($r['type'], array('radio', 'checkbox'))){
			...
			if ($profile[$r['bind']]) {
				if ($r['bind']=='email' && strexists($profile[$r['bind']], 'we7.cc')) {
					$r['default'] = '';
				}else{
		...
		if ($_W['ispost'] || checksubmit('submit')) {
			...
			if(!empty($linkage)){
				$lg = array(
					'l1' => intval($_GPC['linkage1']),
					'l2' => intval($_GPC['linkage2']),
				);
			...
			$row['getadd'] = $_GPC['getadd'];
			$row['getip'] = $_GPC['getip'];
            ...
            foreach ($_GPC as $key => $value) {
                if (strexists($key, 'field_')) {
					...
					/*
					if (in_array($activity['skins'], array('weui', 'weui3', 'weui_pg', 'weui4', 'weui_huahua', 'weui_ju'))) {
						if (in_array($field['type'], array('number', 'text', 'calendar', 'email', 'textarea', 'radio', 'range', 'select', 'image', 'reside', 'checkbox', 'photograph', 'tingli', 'phone', 'tel', 'idcard'))) {
							$entry['data'] = strval($value);
						}
					}else{
						if (in_array($field['type'], array('number', 'text', 'calendar', 'email', 'textarea', 'radio', 'range', 'select', 'image', 'reside', 'photograph', 'tingli', 'phone', 'tel', 'idcard'))) {
							$entry['data'] = strval($value);
                        }
					...
					*/
						if (in_array($field['type'], array('number', 'text', 'calendar', 'email', 'radio', 'textarea', 'range', 'select', 'image', 'reside', 'photograph', 'tingli', 'phone', 'tel', 'idcard'))) {
							$entry['data'] = strval($value);
                        }
						if (in_array($field['type'], array('checkbox'))) {
							if ($field['loc']=='-1') {
			...
			if(pdo_tableexists('dayu_sendkami') && !empty($par['sendkami'])) {
				...
				$kamiinfo = !empty($row['kid']) ? "\\n　卡号：".$sendkami['number']."\\n　密码：".$sendkami['password'] : "";
					$template =array(
						"touser"=>$row['openid'],
						"template_id"=>$activity['k_templateid'],
						"url"=> murl('entry', array('do' => 'mydayu_form', 'op' => 'detail', 'id' =>$row['reid'], 'rerid' => $rerid, 'm' => 'dayu_form'), true, true),
						"topcolor"=>"#FF0000",
						"data"=>array(
							'first'=>array('value'=>urlencode($activity['mfirst']),'color'=>"#743A3A",),
							'keyword1'=>array('value'=>urlencode($member),'color'=>'#000000',),
							'keyword2'=>array('value'=>urlencode($row['mobile']),'color'=>'#000000',),
							'keyword3'=>array('value'=>urlencode($ytime),'color'=>'#000000',),
							'keyword4'=>array('value'=>urlencode($kamiinfo),'color'=>"#FF0000",),
							'remark'=>array('value'=>urlencode($activity['mfoot']),'color'=>"#008000",),
						)
					);
						$this->send_template_message(urldecode(json_encode($template)));
			}
            if (!empty($datas)) {
                ...
                foreach ($datas as $row) {
					...
					$bodyp .= $fields[$row['refid']]['title'] . '：' . $row['data'].'|';
//					$smsbody .= '('.$fields[$row['refid']]['title'].':'.$row['data'].')';
					$smsbody .= $fields[$row['refid']]['title'].':'.$row['data'].'，';
                }
				*/				
                foreach ($datas as $d) {
					if (strstr($d['data'], 'images')) {
						$d['data'] = "有";
			//			$d['data'] = "<img src=".tomedia($d['data']).">";
					}
					/*
					$d['maildata'] = $d['data'];
					if (strstr($d['maildata'], 'images')) {
						$d['maildata'] = "<img src=".tomedia($d['data'])." style='width:50px;height:50px;'>";
					}
					*/
                    $bodym .= '\\n　' . $fields[$d['refid']]['title'] . ':' . $d['data'];
					$body .= '<h4>' . $fields[$d['refid']]['title'] . '：' . $d['data'] . '</h4>';
					$bodyp .= $fields[$d['refid']]['title'] . '：' . $d['data'].'|';
//					$smsbody .= '('.$fields[$d['refid']]['title'].':'.$d['data'].')';
					$smsbody .= $fields[$d['refid']]['title'].':'.$d['data'].'，';
                }
				
				if (pdo_tableexists('dayu_print') && !empty($par['print'])) {
					ihttp_post(murl('entry', array('do' => 'print', 'printid' => $par['print'], 'm' => 'dayu_print'), true, true), array('title' => $activity['title'], 'realname' => $member, 'mobile' => $_GPC['mobile'], 'info' => $bodyp, 'createtime' => $row['createtime']));
				}
				...
				if ($activity['custom_status'] == 0 && $staff && $activity['k_templateid']) {
					if(is_array($staff)) {
						foreach($staff as $s) {
							...
							"data"=>array('first'=>array('value'=>urlencode($activity['kfirst']."\\n"),'color'=>"#743A3A",),
								'keyword1'=>array('value'=>urlencode($member),'color'=>'#000000',),
								'keyword2'=>array('value'=>urlencode($_GPC['mobile']),'color'=>'#000000',),
								'keyword3'=>array('value'=>urlencode($ytime),'color'=>'#000000',),
								'keyword4'=>array('value'=>urlencode($bodym.$voice.$kamiinfo."\\n"),'color'=>"#FF0000",),
								'remark'=>array('value'=>urlencode($activity['kfoot']),'color'=>"#008000",),
								)
							);
//						load()->func('communication');
...
//				mc_credit_update($uid, $behavior['activity'], $activity['credit'], array(0, $activity['title'].'-扣除'.$activity['credit'].$creditnames[$behavior['activity']]['title']));
	...
	public function doMobileConsult(){
        ...
        $rerid = intval($_GPC['rerid']);
        $cid = intval($_GPC['cid']);
        $form = intval($_GPC['form']);
		$isstaff = $this->get_staff($openid,$reid);
		if(empty($cid) || empty($reid) || empty($rerid) || empty($weid) || empty($openid)){
			$this->showMessage('系统错误.',referer(),'error');
		}
		$check = pdo_get($this->tb_info, array('rerid' => $rerid), array());
		if($check['status']=='3'){
			$this->showMessage('已完成，关闭咨询.',referer(),'error');
		}elseif($check['openid']!=$openid && !$isstaff){
			$this->showMessage('非法访问.',referer(),'error');
		}
		if($form=='1' && $isstaff){
			$staff='1';
			$back_url = $this->createMobileUrl('manageform', array('op' => 'detail', 'rerid' => $rerid, 'id' => $reid));
			$title='在线咨询管理';
		}else{
			$staff='0';
			$back_url = $this->createMobileUrl('mydayu_form', array('op' => 'detail', 'rerid' => $rerid, 'id' => $reid));
			$title='在线咨询';		
		}
		$jquery=1;
    ...
    public function get_staff($openid,$reid){
        global $_GPC,$_W;
		return pdo_get($this->tb_staff, array('weid' => $_W['uniacid'], 'openid' => $openid, 'reid' => $reid), array());
    }
	...
	public function doMobileUploads(){
		...
		if ($operation == 'upload') {
			if($type == 'image'){
				...
				$filename = $acc->downloadMedia($media, true);
...
//					$result['filename'] = $filename;
	...
	public function doMobileUpload(){
    	...
    	load()->func('file');@mkdirs($upload_path.$images_path);
...
//			$result['status']=1;
//			$result['msg']='上传成功';
//			$result['images']=$pathname;
//			$result = array('message' => '上传成功', 'status' => 1, 'images' => $pathname);
...
//		checkAuth();
        ...
        if ($operation == 'display') {
			if ($reid) {
				...
				foreach ($rows as &$row) {
					$row['user'] = mc_fansinfo($row['openid'],$acid, $weid);
					$row['kf'] = mc_fansinfo($row['kf'],$acid, $weid);
					$row['status'] = $this->get_status($val['reid'],$val['status']);
					$row['comment'] = !empty($par['comment']) && pdo_tableexists('dayu_comment') && !empty($row['commentid']) ? $this->get_comment($row['commentid']) : '';
...
//				}
//				$dayu_form = pdo_fetchall("SELECT * FROM " . tablename($this->tb_form) . " WHERE reid IN (" . implode(',', $reids) . ")", array(), 'reid');
//			}
			$user_footer=1;
			...
			$row['yuyuetime'] = !empty($row['yuyuetime']) ? date('Y-m-d H:i', $row['yuyuetime']) : '请等待客服受理';
			$row['thumb']   = iunserializer($row['thumb']);
			$row['voices'] = $row['voice'];
			$row['revoices'] = $row['revoice'];
//			$row['rethumbs'] = tomedia($row['rethumb']);
			...
			if(pdo_tableexists('dayu_consult')) {
				$consult = pdo_fetch("SELECT id,createtime FROM ".tablename('dayu_consult')." WHERE infoid = :infoid and openid = :openid and rid = '0' ORDER BY createtime DESC", array(":infoid" => $row['rerid'],":openid" => $openid));
...
//				print_r($consult);
				if(!empty($consult['id']) && $consult['createtime']<$consultr['createtime']){
					$c_tishi = '<span class="weui-badge right" style="margin-left: 5px;position: absolute;top:5px;right:5px;">有新回复</span>';
				}
			}
//			$dayu_form = pdo_fetch("SELECT * FROM " . tablename($this->tb_form) . " WHERE reid = :reid", array(':reid' => $row['reid']));
//			$dayu_form['fields'] = pdo_fetchall("SELECT a.title, a.type, b.data FROM " . tablename($this->tb_field) . " AS a LEFT JOIN " . tablename($this->tb_data) . " AS b ON a.refid = b.refid WHERE a.reid = :reid AND b.rerid = :rerid", array(':reid' => $row['reid'], ':rerid' => $id));
			/*
			$info_rd = pdo_fetchall('SELECT * FROM ' . tablename($this->tb_record) . ' WHERE rerid = :rerid ORDER BY id', array(':rerid' => $row['rerid']));
			foreach($info_rd AS $key => $val){
				$info_rd[$key]['user'] = mc_fansinfo($val['openid']);
			...
			if (pdo_update($this->tb_info, $data, array('reid' => $reid, 'rerid' => $rerid)) === false) {
				...
				if ($_W['account']['level'] == ACCOUNT_SERVICE_VERIFY && !empty($activity['k_templateid'])) {
					$template =array(
						"touser"=>$_GPC['openid'],
						"template_id"=>$activity['k_templateid'],
//						"url"=>$_W['siteroot'] .'app/'.$this->createMobileUrl('manageform', array('name' => 'dayu_form', 'weid' => $_W['uniacid'], 'id' => $reid)),
						"url"=>$_W['siteroot'] .'app/'.$this->createMobileUrl('manageform', array('name' => 'dayu_form', 'weid' => $_W['uniacid'], 'op' => 'detail', 'id' => $reid, 'rerid' => $rerid)),
						"topcolor"=>"#FF0000",
						"data"=>array(
							'first'=>array('value'=>urlencode($activity['kfirst']."\\n"),'color'=>"#743A3A",),
							'keyword1'=>array('value'=>urlencode($content['member']),'color'=>'#000000',),
							'keyword2'=>array('value'=>urlencode($content['mobile']),'color'=>'#000000',),
							'keyword3'=>array('value'=>urlencode(date('Y-m-d H:i:s', $content['createtime'])),'color'=>'#000000',),
							'keyword4'=>array('value'=>urlencode("管理员派单\\n"),'color'=>"#FF0000",),
							'remark'=>array('value'=>urlencode($activity['kfoot']),'color'=>"#008000",),
						)
					);
//					load()->func('communication');
...
//			$isstaff = pdo_get($this->tb_staff, array('weid' => $weid, 'reid' => $reid, 'openid' => $openid), array('id'));
			$isstaff = pdo_get($this->tb_staff, array('weid' => $weid, 'openid' => $openid), array('id'));
			if (!$isstaff){
				$this->showMessage('非法访问！你不是管理员。', $this->createMobileUrl('index'), 'info');
			}
			
//		$where2='reid=:reid and status = 1';
		$where2='weid=:weid and status = 1';
		$params2 = array();
		$params2[':weid'] = $weid;
		if ($activity['guanli']=='0'){
            $where2.=' and kfid = :openid';
            $params2[':openid'] = $openid;			
		}
		$list = pdo_fetchall("SELECT * FROM " . tablename($this->tb_form) . " WHERE $where2 ORDER BY reid DESC", $params2, 'reid');
...
//			$rows = pdo_fetchall("SELECT * FROM " . tablename($this->tb_info) . " WHERE reid = :reid $where ORDER BY rerid DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize, array(':reid' => $reid));
//			$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename($this->tb_info) . " WHERE reid = :reid $where ", array(':reid' => $reid));
			$pager = $this->pagination($total, $pindex, $psize);
			$rows = pdo_fetchall($sql, $params, 'rerid');
			foreach($rows AS $key => $val){
				$rows[$key]['status'] = $this->get_status($val['reid'],$val['status']);
...
//                    $reids[$row['reid']] = $row['reid'];
//                }
//                $dayu_form = pdo_fetchall("SELECT * FROM " . tablename($this->tb_form) . " WHERE reid IN (" . implode(',', $reids) . ") $where", array(), 'reid');
//            }
			$manage_footer=1;
        } elseif ($operation == 'detail') {
			$setting = $this->module['config'];
			...
			if ($openid == $activity['kfid'] || $openid == $row['kf'] || ($activity['guanli']=='1' && $isstaff)) {
				if(pdo_tableexists('dayu_consult')) {
					$consult = pdo_fetch("SELECT id,createtime FROM ".tablename('dayu_consult')." WHERE infoid = :infoid and rid = '0' ORDER BY createtime DESC", array(":infoid" => $row['rerid']));
					$consultr = pdo_fetch("SELECT id,createtime FROM ".tablename('dayu_consult')." WHERE infoid = :infoid and rid = '1' ORDER BY createtime DESC", array(":infoid" => $row['rerid']));
					if(!empty($consult['id']) && $consult['createtime']>$consultr['createtime']){
						$c_tishi = '<span class="weui-badge right" style="margin-left: 5px;position: absolute;top:5px;right:5px;">有新咨询</span>';
					}
				}
				$custom = pdo_fetchall("SELECT * FROM ".tablename($this->tb_custom)." WHERE weid = :weid ORDER BY displayorder DESC", array(':weid' => $weid));
				if (empty($row)) {
...
//				$fansloc = pdo_fetch("SELECT * FROM ".tablename($this->loc_table)." WHERE weid = :weid AND openid = :openid", array(':weid' => $weid, ':openid' => $row['openid']));
				$face = mc_fansinfo($row['openid'],$acid, $weid);
				$dayu_form = pdo_fetch("SELECT * FROM " . tablename($this->tb_form) . " WHERE reid = :reid", array(':reid' => $row['reid']));
				$dayu_form['content'] = htmlspecialchars_decode($dayu_form['content']);
//				$dayu_form['fields'] = pdo_fetchall("SELECT a.title, a.type, b.data FROM " . tablename($this->tb_field) . " AS a LEFT JOIN " . tablename($this->tb_data) . " AS b ON a.refid = b.refid WHERE a.reid = :reid AND b.rerid = :rerid", array(':reid' => $row['reid'], ':rerid' => $rerid));
				$sql = 'SELECT * FROM ' . tablename($this->tb_field) . ' WHERE `reid`=:reid ORDER BY displayorder DESC, refid DESC';
				$params = array();
				$params[':reid'] = $row['reid'];
				$fields = pdo_fetchall($sql, $params);
...
//					$record['revoice']= !empty($_GPC['revoice']) ? $setting['qiniu']['host'].'/'.$_GPC['revoice'] : '';
					$record['revoice']= empty($row['revoice']) ? $_GPC['revoice'] : $row['revoice'];
//					$record['rethumb']= !empty($_GPC['rethumb']) ? $_GPC['rethumb'] : '';
					...
					if($dayu_form['custom_status'] == 1){
						...
						$template =array(
							...
							"topcolor"=>"#FF0000",
							"data"=>array(
								'first'=>array('value'=>urlencode($dayu_form['mfirst']),'color'=>"#743A3A",),
								'keyword1'=>array('value'=>urlencode($row['member']),'color'=>'#000000',),
								'keyword2'=>array('value'=>urlencode($row['mobile']),'color'=>'#000000',),
								'keyword3'=>array('value'=>urlencode($_GPC['yuyuetime']),'color'=>'#000000',),
								'keyword4'=>array('value'=>urlencode($huifu),'color'=>"#FF0000",),
								'remark'=>array('value'=>urlencode($dayu_form['mfoot']),'color'=>"#008000",),
							)
						);
//						load()->func('communication');
						$this->send_template_message(urldecode(json_encode($template)));
...
//		$account_code = "account_weixin_code";
    ...
    public function doMobilechangeAjax() {
        ...
        if (!empty($id)) {
//			$url=$_W['siteroot'] .'app/'.$this->createMobileUrl('mydayu_form', array('op' => 'detail', 'rerid' => $id, 'id' => $reid));
			...
			$data = array (
				...
				'keyword4' => array('value' => $activity['state3'],),
				'remark' => array('value' => "\n".$activity['mfoot'],'color'=>"#008000")
			);
			$acc = WeAccount::create($_W['acid']);
    ...
    function pagination($tcount, $pindex, $psize = 15, $url = '', $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => '')) {
        ...
        if($pdata['cindex'] > 1) {
            ...
            $html .= "<div class=\"pager-left\">";
            $html .= "<div class=\"pager-pre\" style=\"width:100%\"><a href=\"###\">第一页</a></div>";
            $html .= "</div>";
        }
        ...
        if($pdata['cindex'] < $pdata['tpage']) {
            ...
            $html .= "<div class=\"pager-right\">";
            $html .= "<div class=\"pager-next\" style=\"width:100%\"><a href=\"###\">尾页</a></div>";
            $html .= "</div>";
        }
...
private function checkauth3($openid,$nickname,$headimgurl) {
	global $_W, $engine;
...
//	public function get_fields($refid,$reid){
//		global $_GPC,$_W;
//		return pdo_fetch("SELECT refid,title,type FROM ".tablename($this->tb_field)." WHERE refid = :refid and reid = :reid LIMIT 1", array(":refid" => $refid, ":reid" => $reid));
//	}
