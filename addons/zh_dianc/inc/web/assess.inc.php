...
$pagesize=10;
$sql="select * from ". tablename("wpdc_assess").  "WHERE uniacid=:uniacid  and content LIKE :name";
$select_sql =$sql." LIMIT " .($pageindex - 1) * $pagesize.",".$pagesize;
$list = pdo_fetchall($select_sql,array(':uniacid'=>$_W['uniacid'],':name'=>$where));	   
$total = pdo_fetchcolumn("select count(*) from ".tablename("wpdc_assess").  "WHERE uniacid=:uniacid  and content LIKE :name",array(':uniacid'=>$_W['uniacid'],':name'=>$where));	
