...
$url=substr($str,0,$n);
	...
	if($order['state']==1){
		...
		if($order['coupons_id']){
          ...
          $level=pdo_getall('zhvip_level',array('uniacid'=>$order['uniacid'],'level >'=>$user['grade']),array(),'','level asc');
          if($level){
            $money=$user['level_cumulative'];
            for($i=0;$i<count($level);$i++){
              if(($money-$level[$i]['threshold'])>=0){
                ...
                if($i>0){
                 pdo_update('zhvip_user',array('grade'=>$level[$i-1]['id'],'level_cumulative'=>$money),array('id'=>$order['user_id'])); 
               }
               break;
             }
           }
		...
		///////////加积分//////////////
         ...
         if(!$czorder['store_id']){
         	$level=pdo_get('zhvip_level',array('uniacid'=>$czorder['uniacid']),array(),'','level ASC');
			  ...
			  $data2['vip_code'] =str_shuffle(date('YmdHis').rand(1111,9999));
