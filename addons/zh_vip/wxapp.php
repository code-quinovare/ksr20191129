...
//短信验证码
	...
	//优惠券列表
				public function doPageCoupons(){
					...
					$sql2 = "select a.*,b.name as level_name from " . tablename("zhvip_coupons") . " a"  . " left join " . tablename("zhvip_level") . " b on b.level=a.level_type WHERE a.uniacid=".$_W['uniacid']." and a.store_id in (0,".$_GPC['store_id'].") and UNIX_TIMESTAMP(a.end_time) >".time();
...
//商家登录
 public function doPageStoreLogin(){
	 ...
	 $res=pdo_get('zhvip_account',array('account'=>$user,'pwd'=>$password,'uniacid'=>$_W['uniacid']));
	 if($res){
		 echo json_encode($res);
	 }else{
		 echo  '2';
...
public function doPageExchange(){
	...
	//兑换明细
			...
			public function doPageAddDefault(){
				...
				$res=pdo_update('zhvip_useradd',$data,array('id'=>$_GPC['id']));
				if($res){
					pdo_update('zhvip_useradd',array('is_default'=>2),array('id !='=>$_GPC['id']));
					echo '1';
				}else{
					echo '2'; 
				}
