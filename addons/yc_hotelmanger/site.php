...
class Yc_hotelmangerModuleSite extends WeModuleSite
{
	...
	public $devices = array(array('isdel' => 0, 'value' => '有线上网'), array('isdel' => 0, 'isshow' => 0, 'value' => 'WIFI无线上网'), array('isdel' => 0, 'isshow' => 0, 'value' => '可提供早餐'), array('isdel' => 0, 'isshow' => 0, 'value' => '免费停车场'), array('isdel' => 0, 'isshow' => 0, 'value' => '会议室'), array('isdel' => 0, 'isshow' => 0, 'value' => '健身房'), array('isdel' => 0, 'isshow' => 0, 'value' => '游泳池'));
	public $hotelroom = array(array('isdel' => 0, 'value' => '房间面积', 'danwei' => '平方米'), array('isdel' => 0, 'isshow' => 0, 'value' => '楼层', 'danwei' => '平方米'), array('isdel' => 0, 'isshow' => 0, 'value' => '床位', 'danwei' => '2米大床'), array('isdel' => 0, 'isshow' => 0, 'value' => '是否加床', 'danwei' => '加床说明'), array('isdel' => 0, 'isshow' => 0, 'value' => '无烟', 'danwei' => '无烟说明'), array('isdel' => 0, 'isshow' => 0, 'value' => '宽带', 'danwei' => '无线（有线）'), array('isdel' => 0, 'isshow' => 0, 'value' => '其他', 'danwei' => '其他信息'));
	public $_weid = '';
	public $psize = 15;
	public $openid = '';
	public $menu = '';
	...
	public function get_sendorderok($seearr, $order_on)
	{
		...
		$data = array('first' => array('value' => $order['order_name'] . '先生/女士，你的订房订单已确认。', 'color' => '#DC143C'), 'keyword1' => array('value' => $order['ordertime'], 'color' => '#173177'), 'keyword2' => array('value' => $order['goods_name'] . $order['yu_legth'] . '间,时间' . date('Y-m-d', $order['sintdate']) . '-' . date('Y-m-d', $order['soutdate']), 'color' => '#173177'), 'remark' => array('value' => "\n" . '订单信息：金额：￥' . $order['totalcpice'] . '元' . "\n" . '订单号：' . $order['order_on'] . "\n" . '期待您的光临！如有疑问请致电' . $order['title'] . '客服电话：' . $order['hphone'] . '！', 'color' => '#228B22'));
		$tjsrt['data'] = $data;
	...
	public function get_hotelordergl($seearr, $order_on)
	{
		...
		if ($order['mode'] == 'wechat') {
			...
			if ($order['mode'] == 'credit') {
				$type = '余额支付';
			}
		}
		foreach ($tpluserarr as $val) {
			$tjsrt = array('template_id' => $seearr['xindan'], 'touser' => $val, 'url' => '', 'topcolor' => '#228B22');
			$data = array('first' => array('value' => $order['title'] . '产生新订单啦！', 'color' => '#173177'), 'keyword1' => array('value' => $order['ordertime'], 'color' => '#173177'), 'keyword2' => array('value' => $order['goods_name'] . $order['yu_legth'] . '间,时间' . date('Y-m-d', $order['sintdate']) . '-' . date('Y-m-d', $order['soutdate']), 'color' => '#173177'), 'remark' => array('value' => '订单信息：' . "\n" . '客户姓名：' . $order['order_name'] . "\n" . '手机号：' . $order['phone'] . "\n" . '金额：￥' . $order['totalcpice'] . '元' . "\n" . '订单号：' . $order['order_on'] . "\n" . '支付方式：' . $type . "\n" . '请做好服务工作!', 'color' => '#DC143C'));
			$tjsrt['data'] = $data;
			$jsonData = json_encode($tjsrt);
			load()->func("communication");
			$acessToken = $this->getAccessToken();
	...
	public function get_hotelroomtui($seearr, $order_id, $type = 1)
	{
		...
		foreach ($tpluserarr as $val) {
			...
			if ($type == 1) {
				$data = array('first' => array('value' => '客人因行程改变已退订订单。', 'color' => '#173177'), 'keyword1' => array('value' => $order['order_on'], 'color' => '#173177'), 'keyword2' => array('value' => $order['title'], 'color' => '#173177'), 'keyword3' => array('value' => $order['goods_name'] . $order['yu_legth'] . '间。', 'color' => '#173177'), 'keyword4' => array('value' => date('Y-m-d', $order['sintdate']), 'color' => '#173177'), 'keyword5' => array('value' => date('Y-m-d', $order['soutdate']), 'color' => '#173177'), 'remark' => array('value' => '订单已退订 请取消预订登记，不用保留客房。', 'color' => '#173177'));
			} else {
				$data = array('first' => array('value' => '客人因行程改变已申请订单退款。', 'color' => '#173177'), 'keyword1' => array('value' => $order['order_on'], 'color' => '#173177'), 'keyword2' => array('value' => $order['title'], 'color' => '#173177'), 'keyword3' => array('value' => $order['goods_name'] . $order['yu_legth'] . '间。', 'color' => '#173177'), 'keyword4' => array('value' => date('Y-m-d', $order['sintdate']), 'color' => '#173177'), 'keyword5' => array('value' => date('Y-m-d', $order['soutdate']), 'color' => '#173177'), 'remark' => array('value' => '订单已申请退款 请取消预订登记，后台确认退款。', 'color' => '#173177'));
			}
			$tjsrt['data'] = $data;
			$jsonData = json_encode($tjsrt);
			load()->func("communication");
	...
	public function get_hotelutud($seearr, $order_id)
	{
		...
		$data = array('first' => array('value' => $order['title'] . '预定退房成功！', 'color' => '#173177'), 'keyword1' => array('value' => $order['title'] . $order['goods_name'] . $order['yu_legth'] . '间', 'color' => '#173177'), 'keyword2' => array('value' => $order['totalcpice'] . '（进店付款）', 'color' => '#173177'), 'keyword3' => array('value' => '退房成功', 'color' => '#173177'), 'remark' => array('value' => '感谢您的惠顾，' . $order['title'] . '欢迎您下次光临！客服电话：' . $order['phone'], 'color' => '#173177'));
		$tjsrt['data'] = $data;
		$jsonData = json_encode($tjsrt);
		load()->func("communication");
		$acessToken = $this->getAccessToken();
	...
	public function get_hoteluserkk($seearr, $order_id, $type = 1)
	{
		...
		if ($type == 1) {
			$data = array('first' => array('value' => '您好，您的' . $order['title'] . '订单正在退款申请中，请耐心等待！', 'color' => '#173177'), 'keyword1' => array('value' => $order['title'], 'color' => '#173177'), 'keyword2' => array('value' => $order['goods_name'], 'color' => '#173177'), 'keyword3' => array('value' => $order['yu_legth'] . '间', 'color' => '#173177'), 'keyword4' => array('value' => $order['totalcpice'] . '元', 'color' => '#173177'), 'keyword5' => array('value' => $stype, 'color' => '#173177'), 'remark' => array('value' => '如有疑问请致电' . $order['title'] . '  客服电话：' . $order['hphone'], 'color' => '#173177'));
		} else {
			$data = array('first' => array('value' => '您好，您的' . $order['title'] . '订单退款成功，自动存入会员卡余额！', 'color' => '#173177'), 'keyword1' => array('value' => $order['title'], 'color' => '#173177'), 'keyword2' => array('value' => $order['goods_name'], 'color' => '#173177'), 'keyword3' => array('value' => $order['yu_legth'] . '间', 'color' => '#173177'), 'keyword4' => array('value' => $order['totalcpice'] . '元', 'color' => '#173177'), 'keyword5' => array('value' => $stype, 'color' => '#173177'), 'remark' => array('value' => '如有疑问请致电' . $order['title'] . '  客服电话：' . $order['hphone'], 'color' => '#173177'));
		}
		$tjsrt['data'] = $data;
		$jsonData = json_encode($tjsrt);
		load()->func("communication");
