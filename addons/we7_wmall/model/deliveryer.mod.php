...
function icheckdeliveryer() 
{
	...
	if (!(empty($_W['deliveryer']))) 
	{
		if (empty($_W['deliveryer']['openid_wxapp'])) 
		{
...
function deliveryer_fetch($value, $field = 'id') 
{
	...
	if (!(empty($deliveryer))) 
	{
		...
		{
			$deliveryer['extra'] = array('accept_wechat_notice' => 1, 'accept_voice_notice' => 1);
		}
		...
		{
			$deliveryer['fee_getcash'] = array('get_cash_fee_limit' => 1, 'get_cash_fee_rate' => 0, 'get_cash_fee_min' => 0, 'get_cash_fee_max' => 0, 'get_cash_period' => 0);
		}
		$tips = array('休息中', '接单中');
...
function deliveryer_update_credit2($deliveryer_id, $fee, $trade_type, $extra, $remark = '', $order_type = 'order') 
{
	...
	$log = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'deliveryer_id' => $deliveryer_id, 'order_type' => $order_type, 'trade_type' => $trade_type, 'extra' => $extra, 'fee' => $fee, 'amount' => $now_amount, 'addtime' => TIMESTAMP, 'stat_month' => date('Ym', TIMESTAMP), 'remark' => $remark);
...
function deliveryer_getcash_transfers($id) 
{
	...
	if ($_W['is_agent'] && (0 < $deliveryer['agentid'])) 
	{
		mload()->model('agent');
