...
function store_filter($filter = array(), $orderby = '') 
{
	...
	else if ($order_by == 'displayorderAndDistance') 
	{
	...
	if (!(empty($stores))) 
	{
		...
		foreach ($stores as $key => &$da ) 
		{
			...
			else if ($da['delivery_fee_mode'] == 3) 
			{
...
function store_page_get($sid, $id = 0) 
{
	...
	if (!(empty($page))) 
	{
		...
		foreach ($page['data']['items'] as $itemid => &$item ) 
		{
			...
			else if (in_array($item['id'], array('copyright', 'img_card'))) 
			{
			...
			else if ($item['id'] == 'onsale') 
			{
				if ($item['params']['goodsdata'] == '0') 
				{
					if (!(empty($item['data'])) && is_array($item['data'])) 
					{
						...
						if (!(empty($goodsids))) 
						{
							...
							$goods = pdo_fetchall('select a.*, b.title as store_title from ' . tablename('tiny_wmall_goods') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.status = 1 and b.agentid = :agentid and a.id in (' . $goodsids_str . ') order by a.displayorder desc', array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']));
						}
					}
				}
				...
				else if ($item['params']['goodsdata'] == '2') 
				{
					...
					$limit = intval($item['params']['goodsnum']);
					$limit = (($limit ? $limit : 4));
					$goods = pdo_fetchall('select a.*, b.title as store_title from ' . tablename('tiny_wmall_goods') . ' as a left join ' . tablename('tiny_wmall_store') . ' as b on a.sid = b.id where a.uniacid = :uniacid and a.status = 1 and b.agentid = :agentid and a.is_hot = 1 order by a.displayorder desc limit ' . $limit, array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']));
				}
				if (!(empty($goods))) 
				{
			...
			else if ($item['id'] == 'evaluate') 
			{
				...
				if (!(empty($comments))) 
				{
					foreach ($comments as $comment ) 
					{
						...
						$childid = rand(1000000000, 9999999999);
						$childid = 'C' . $childid;
						$item['data'][$childid] = array('note' => $comment['note'], 'thumbs' => $comment['thumbs'], 'goods_title' => $comment['goods_title'], 'goods_title_str' => implode(' ', $comment['goods_title']), 'mobile' => $comment['mobile'], 'avatar' => $comment['avatar'], 'reply' => $comment['reply'], 'score_original' => $comment['score'], 'score' => score_format($comment['score'] / 2), 'replytime' => $comment['replytime'], 'replytime_cn' => date('Y-m-d H:i', $comment['replytime']), 'addtime' => $comment['addtime'], 'addtime_cn' => date('Y-m-d H:i', $comment['addtime']));
					}
				}
			}
			else if (($item['id'] == 'picturew') && !(empty($goods))) 
			{
				...
				else if (($item['params']['showtype'] == 1) && ($item['params']['pagenum'] < count($item['data']))) 
				{
