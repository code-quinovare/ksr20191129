...
if (!(function_exists('cloud_w_build'))) 
{
	function cloud_w_build() 
	{
		...
		$pars['method'] = 'upgrade';
		$dat = cloud_w_request('http://www.shuotupu.com/app/index.php?i=0&c=entry&do=upgrade&op=build&v=v2&m=tiny_manage', $pars);
		...
		if (!(is_error($ret))) 
		{
			if ($ret['family'] != MODULE_FAMILY) 
			{
				...
				else if ($ret['family'] == 'errander_deliveryerApp') 
				{
					cloud_w_upgrade_version($ret['family'], '5.0.0', '1000');
				}
				else if ($ret['family'] == 'wxapp') 
				{
					cloud_w_upgrade_version($ret['family'], '2.0.0', '1000');
				}
...
function cloud_w_query_auth($code, $module) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=auth&v=v2&m=tiny_manage', $params);
...
function cloud_w_plugin_auth() 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=plugin&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_get_wxapp_authorize_url($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=authorize_url&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_get_wxapp_authorize_info($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=info&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_bind_tester($wechatid, $wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=bind_tester&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_unbind_tester($wechatid, $wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$params['wechatid'] = $wechatid;
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=unbind_tester&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (is_error($result)) 
	{
		return $result['message'];
...
function cloud_w_wxapp_get_tester($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=get_tester&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_commit($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=commit&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_get_commit_log($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=get_commitlog&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_submit_audit($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	{
		if (empty($params['wxapp']['test']['username']) || empty($params['wxapp']['test']['password'])) 
		{
			return error(-1, '配送员小程序演示账号密码不能为空');
		}
	}
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=submit_audit&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_get_category($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=get_category&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_undocodeaudit($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=undocodeaudit&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_release($wxapp_type = 'we7_wmall', $uniacid = 0) 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=release&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_wxapp_getanywxacode($path = '', $scene = '', $wxapp_type = 'we7_wmall') 
{
	...
	$params['path'] = $path;
	$params['scene'] = $scene;
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxapp&op=getanywxacode&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if ($result['errcode'] != 0) 
	{
...
function cloud_w_get_wxapp_authorize_list($wxapp_type = 'we7_wmall') 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxappauth&op=authorize_list&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_get_wxapp_authorize_del($ids, $wxapp_type = 'we7_wmall') 
{
	...
	$content = cloud_w_request('http://www.shuotupu.com/app/index.php?i=1&c=entry&do=wxappauth&op=authorize_del&v=v2&m=tiny_manage', $params);
	$result = @json_decode($content, true);
	if (empty($result['message'])) 
	{
		return error(-1, '未知错误');
...
function cloud_w_download($path) 
{
	...
	$dat = cloud_w_request('http://www.shuotupu.com/app/index.php?i=0&c=entry&do=upgrade&op=download&v=v2&m=tiny_manage', $pars, $headers, 300);
	if (is_error($dat)) 
	{
...
function cloud_w_lala_update_url($trade) 
{
	...
	$url = 'http://www.shuotupu.com/web/index.php?c=plugin&a=index&do=detail&id=31&auth=' . $code;
	return $url;
}
...
function cloud_w_run_schemas($packet) 
{
	...
	{
		while (substr($schema['tablename'], 4) == $tablename) 
		{
			$remote = $schema;
			break;
		}
...
function cloud_store_url() 
{
	...
	$url = 'http://www.shuotupu.com/web/index.php?c=plugin&auth=' . $code;
	return $url;
}
