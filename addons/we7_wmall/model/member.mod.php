...
function icheckauth($force = true) 
{
	...
	{
		$result = array('errno' => 41000, 'message' => '需要获取openid', 'sessionid' => $_W['session_id'], 'oauthurl' => imurl('system/common/vuesession', array('state' => $_W['session_id'], 'from' => 'vue'), true));
	...
	if ($force) 
	{
		...
		{
			$result = array('errno' => 41009, 'message' => '请先登录', 'sessionid' => $_W['session_id'], 'oauthurl' => imurl('system/common/vuesession/userinfo', array('state' => $_W['session_id'], 'from' => 'vue'), true));
			imessage($result, '', 'ajax');
		}
		else if (is_qianfan()) 
		{
...
function member_fetch_available_address($sidOrStore) 
{
	...
	if (!(empty($address)) && ($store['delivery_fee_mode'] == 2)) 
	{
		...
		else if ($store['delivery_price_extra']['calculate_distance_type'] == 2) 
		{
			$dist = floor($dist);
		}
...
function member_group_update($wx_tpl = false) 
{
	...
	else if ($config_member['group_update_mode'] == 'order_count') 
	{
		$condition .= ' and status = 5';
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
		$result = intval($result);
	}
	else if ($config_member['group_update_mode'] == 'delivery_money') 
	{
	...
	else if ($config_member['group_update_mode'] == 'delivery_count') 
	{
		$condition .= ' and status = 3';
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_errander_order') . $condition, $params);
		$result = intval($result);
	}
	else if ($config_member['group_update_mode'] == 'count_money') 
	{
