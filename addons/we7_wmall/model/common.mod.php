...
function set_system_config($key, $value) 
{
	...
	else if ($counts == 3) 
	{
		...
		else if (!(is_array($sysset[$keys[0]][$keys[1]]))) 
		{
...
function get_available_payment($order_type = '', $sid = 0, $all = false, $orderType = 1) 
{
	...
	else 
	{
		...
		else if ($order_type == 'superredpacket_meal') 
		{
			$index = array_search('delivery', $payment);
			if ($index !== false) 
			{
...
function get_agent_perm($key = '', $agentid = 0) 
{
	...
	$agent['plugins'] = array('errander', 'bargain');
	if (!(empty($key))) 
	{
		return $agent[$key];
	}
...
function check_perm($perm, $redirct = false) 
{
	...
	if ($redirct) 
	{
		$perms_init = array('dashboard.index', 'merchant.store', 'order.takeout', 'statcenter.takeout', 'paycenter.paybill', 'merchant.store', 'service.comment', 'deliveryer.account', 'clerk.account', 'member.index', 'config.mall', 'errander.index', 'bargain.index', 'deliveryCard.index', 'qianfanApp.index', 'majiaApp.index', 'shareRedpacket.index', 'freeLunch.index', 'diypage.index', 'ordergrant.index', 'superRedpacket.index', 'creditshop.index', 'agent.index');
...
{
	$all_perms = array( 'dashboard' => array( 'title' => '概括', 'perms' => array('dashboard.index' => '运营概况', 'dashboard.ad' => '全屏引导页', 'dashboard.slide' => '幻灯片', 'dashboard.nav' => '导航图标', 'dashboard.notice' => '公告', 'dashboard.cube' => '图片魔方') ), 'order' => array( 'title' => '订单', 'perms' => array('order.takeout' => '外卖', 'order.takeoutNew' => '未完成（简洁）', 'order.tangshi' => '店内', 'order.dispatch' => '调度中心-待指派', 'order.records' => '调度中心-接单统计/接单记录', 'order.neworder' => '代客下单') ), 'paycenter' => array( 'title' => '收银台', 'perms' => array('paycenter.paybill' => '买单订单') ), 'statcenter' => array( 'title' => '数据', 'perms' => array('statcenter.takeout' => '外卖统计', 'statcenter.delivery' => '配送统计') ), 'merchant' => array( 'title' => '商户', 'perms' => array('merchant.store' => '商户列表', 'merchant.account' => '商户账户', 'merchant.getcash' => '申请提现', 'merchant.current' => '账户明细', 'merchant.settle' => '入驻', 'merchant.newsCategory' => '资讯分类', 'merchant.news' => '资讯列表', 'merchant.ad' => '广告', 'merchant.notice' => '公告列表', 'merchant.report' => '投诉列表') ), 'service' => array( 'title' => '售后', 'perms' => array('service.comment' => '用户评价') ), 'deliveryer' => array( 'title' => '配送员', 'perms' => array('deliveryer.account' => '入驻列表', 'deliveryer.plateform' => '平台配送员', 'deliveryer.getcash' => '申请提现', 'deliveryer.current' => '账户明细', 'deliveryer.cover' => '注册&登陆') ), 'clerk' => array( 'title' => '店员', 'perms' => array('clerk.account' => '店员列表', 'clerk.cover' => '注册&登陆') ), 'member' => array( 'title' => '顾客', 'perms' => array('member.index' => '顾客概况', 'member.list' => '顾客列表', 'member.address' => '顾客地址', 'member.coupon' => '顾客代金券', 'member.redpacket' => '顾客红包') ), 'config' => array( 'title' => '设置', 'perms' => array('config.mall' => '平台', 'config.trade' => '交易', 'config.notice' => '消息推送', 'config.sms' => '短信平台', 'config.store' => '商户', 'config.takeout' => '外卖', 'config.deliveryer' => '配送员', 'config.cover' => '入口', 'config.label' => '商户标签', 'config.report' => '商户举报类型', 'config.help' => '常见问题') ) );
	if (check_plugin_perm('errander')) 
	{
		$all_perms['errander'] = array( 'title' => '跑腿', 'perms' => array('errander.order' => '订单', 'errander.statcenter' => '跑腿统计', 'errander.statDelivery' => '配送统计', 'errander.category' => '跑腿分类', 'errander.config' => '跑腿设置', 'errander.cover' => '入口设置') );
	}
	...
	{
		$all_perms['creditshop'] = array( 'title' => '积分兑换', 'perms' => array('creditshop.order' => '兑换记录', 'creditshop.config' => '系统设置', 'creditshop.cover' => '入口设置') );
	}
	...
	{
		$all_perms['superRedpacket'] = array( 'title' => '超级红包', 'perms' => array('superRedpacket.grant' => '发放红包', 'superRedpacket.share' => '分享红包') );
		if (check_plugin_perm('superRedpacket_meal')) 
		{
			$all_perms['superRedpacket']['perms']['superRedpacket.meal'] = '套餐红包';
			$all_perms['superRedpacket']['perms']['superRedpacket.mealOrder'] = '购买记录';
		}
	}
	if (check_plugin_perm('diypage')) 
	{
		$all_perms['diypage'] = array( 'title' => '平台装修', 'perms' => array('diypage.menu' => '自定义菜单', 'diypage.mall' => '商城设置', 'diypage.danmu' => '订单弹幕', 'diypage.diy' => '页面管理', 'diypage.template' => '模板管理') );
	}
	if (check_plugin_perm('creditshop')) 
	{
		$all_perms['creditshop'] = array( 'title' => '积分商城', 'perms' => array('creditshop.order' => '兑换记录', 'creditshop.adv' => '幻灯片', 'creditshop.config' => '会员卡设置', 'creditshop.goods' => '商品管理', 'creditshop.cover' => '入口设置', 'creditshop.category' => '商品分类') );
	}
	return $all_perms;
}
?>