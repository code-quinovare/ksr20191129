...
function agent_update_account($id, $fee, $trade_type, $extra, $remark = '', $order_type = 'takeout') 
{
	...
	$log = array('uniacid' => $_W['uniacid'], 'agentid' => $id, 'trade_type' => $trade_type, 'order_type' => $order_type, 'extra' => $extra, 'fee' => $fee, 'amount' => $now_amount, 'addtime' => TIMESTAMP, 'remark' => $remark);
...
function sys_notice_agent_getcash($agentid, $getcash_log_id, $type = 'apply', $note = '') 
{
	...
	else if ($type == 'borrow_openid') 
	{
		...
		$remark = array('申请门店: ' . $agent['title'], '账户类型: 微信', '请点击该消息进行授权,否则无法进行提现。如果疑问，请联系平台管理员');
