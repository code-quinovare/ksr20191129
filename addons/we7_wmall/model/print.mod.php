...
function print_add_order($printDeviceInfo = array(), $order = array()) 
{
	...
	else if ($printDeviceInfo['type'] == 'xixun') 
	{
		...
		if (empty($order['print_sn'])) 
		{
			...
			pdo_update('tiny_wmall_order', array('print_sn' => $print_sn), array('id' => $order['id']));
		}
	...
	if (in_array($printDeviceInfo['type'], array('feie', '365'))) 
	{
		...
		if ($printDeviceInfo['type'] == 'feie') 
		{
			pdo_update('tiny_wmall_order', array('print_sn' => trim($result['orderindex'])), array('id' => $order['id']));
		}
	}
	else if ($printDeviceInfo['type'] == 'feie_new') 
	{
		...
		if ($result['msg'] == 'ok') 
		{
			pdo_update('tiny_wmall_order', array('print_sn' => trim($result['data'])), array('id' => $order['id']));
			return true;
		}
