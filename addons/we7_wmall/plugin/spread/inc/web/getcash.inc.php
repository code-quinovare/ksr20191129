...
if ($op == 'transfers') 
{
	...
	$params = array('partner_trade_no' => $log['trade_no'], 'openid' => $log['account']['openid'], 'check_name' => 'FORCE_CHECK', 're_user_name' => $log['account']['realname'], 'amount' => $log['final_fee'] * 100, 'desc' => $log['account']['realname'] . date('Y-m-d H:i', $log['addtime']) . '配送费提现申请');
	mload()->classs('wxpay');
