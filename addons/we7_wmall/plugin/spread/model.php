...
function get_spread($spread_id = 0) 
{
	...
	$fields = array('uid', 'mobile', 'avatar', 'is_spread', 'spreadcredit2', 'spread1', 'spread2', 'spread_groupid', 'spread_groupid_change_from', 'spread_status', 'spreadtime');
...
function spread_group_update($spread_id = 0, $wx_tpl = false) 
{
	...
	else if ($config['group_update_mode'] == 'order_money_1') 
	{
	...
	else if ($config['group_update_mode'] == 'order_count') 
	{
		$condition .= ' and is_pay = 1 and status = 5';
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
	}
	else if ($config['group_update_mode'] == 'order_count_1') 
	{
		$condition .= ' and is_pay = 1 and status = 5 and spread1 = :spread';
		$params[':spread'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
	}
	else if ($config['group_update_mode'] == 'self_order_money') 
	{
	...
	else if ($config['group_update_mode'] == 'self_order_count') 
	{
		$condition .= ' and is_pay = 1 and status =5 and uid = :uid';
		$params[':uid'] = $spread_id;
		$result = pdo_fetchcolumn('select count(*) from' . tablename('tiny_wmall_order') . $condition, $params);
	}
	else if ($config['group_update_mode'] == 'down_count') 
	{
	...
	else if ($config['group_update_mode'] == 'down_count_1') 
	{
	...
	{
		if ($config['admin_update_rules'] == 'keep') 
		{
			$group_id = $spread_info['spread_groupid'];
		}
		else if ($result < $group_condition_now) 
		{
			$group_id = $spread_info['spread_groupid'];
		}
	}
...
function sys_notice_spread_down($uid, $type, $extra = array()) 
{
	...
	if ($type == 'pseudo_down') 
	{
		if (!(empty($spread1['openid']))) 
		{
			...
			if (empty($member['spreadfixed'])) 
			{
				...
				else if ($config_spread['relate']['become_child'] == 2) 
				{
	...
	else if ($type == 'new_down') 
	{
		...
		else if ($extra['channel'] == 'order_comment') 
		{
	...
	$remark = implode("\n", $remark);
	$send = array( 'first' => array('value' => $tips, 'color' => '#ff510'), 'keyword1' => array('value' => $member['uid'], 'color' => '#ff510'), 'keyword2' => array('value' => date('Y-m-d H:i'), 'color' => '#ff510'), 'remark' => array('value' => $remark, 'color' => '#ff510') );
...
function spread_update_credit2($spread_id, $fee, $extra = array()) 
{
	...
	$log = array('uniacid' => $_W['uniacid'], 'spreadid' => $spread_id, 'trade_type' => $extra['trade_type'], 'extra' => $extra['extra'], 'fee' => $fee, 'amount' => $now_amount, 'addtime' => TIMESTAMP, 'remark' => $extra['remark']);
...
function spread_order_balance($order_id) 
{
	...
	if ($balance == 1) 
	{
		if (!(empty($commission))) 
		{
			...
			if (0 <= $fee_spread1) 
			{
				$extra = array('trade_type' => 1, 'extra' => $order['id']);
				if ($commission['commission1_type'] == 'ratio') 
				{
					$extra['remark'] = '订单号:' . $order['id'] . ', 一级下线佣金费率:' . $commission['spread1_rate'] . ', 佣金:' . $fee_spread1 . '元';
				}
				else if ($commission['commission1_type'] == 'fixed') 
				{
					$extra['remark'] = '订单号:' . $order['id'] . ', 一级下线佣金:' . $fee_spread1 . '元';
				}
			...
			if (0 <= $fee_spread2) 
			{
				...
				{
					$extra['remark'] = '订单号:' . $order['id'] . ', 二级下线佣金费率:' . $commission['spread2_rate'] . ', 佣金:' . $fee_spread2 . '元';
				}
				else if ($commission['commission2_type'] == 'fixed') 
				{
					$extra['remark'] = '订单号:' . $order['id'] . ', 二级下线佣金:' . $fee_spread2 . '元';
				}
...
function member_spread_confirm($order_id) 
{
	...
	else if ($config_spread_relate['become_child'] == 2) 
	{
		if (!(empty($order['oid']))) 
		{
			$update['spreadfixed'] = 1;
