...
if ($op == 'index') 
{
	...
	if (!(empty($params))) 
	{
		...
		else if (!(empty($params['buyaddress']))) 
		{
	...
	$result = array('diy' => $diypage['diypage'], 'addresses' => $addresses, 'redPackets' => redPacket_available($order['delivery_fee'], array(), 'paotui'), 'order' => $order, 'buyaddress_id' => $buyaddress['id'], 'buyaddress' => $buyaddress, 'acceptaddress_id' => $acceptaddress['id'], 'acceptaddress' => $acceptaddress);
