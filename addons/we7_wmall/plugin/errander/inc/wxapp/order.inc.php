...
if ($op == 'list') 
{
	...
	$orders = pdo_fetchall('select * from ' . tablename('tiny_wmall_errander_order') . $condition . ' order by id desc limit 7', $params, 'id');
	$min = 0;
	if (!(empty($orders))) 
	{
		...
		foreach ($orders as &$row ) 
		{
			...
			{
				$category = pdo_get('tiny_wmall_errander_category', array('uniacid' => $_W['uniacid'], 'id' => $row['order_cid']), array('title'));
...
if ($op == 'detail') 
{
	...
	$result = array('order' => $order, 'deliveryer' => $deliveryer, 'log' => $log, 'logs' => $logs, 'maxid' => $maxid, 'minid' => $minid, 'refund_logs' => $refund_logs, 'refundmaxid' => $refundmaxid);
...
if ($op == 'new_list') 
{
	...
	if (!(empty($orders))) 
	{
		...
		foreach ($orders as &$row ) 
		{
			...
			if ($row['data']['version'] == 'version_diy') 
			{
				$category = pdo_get('tiny_wmall_errander_page', array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'id' => $row['order_cid']), array('name', 'thumb'));
				$row['title'] = $category['name'];
			}
			else 
			{
			...
			$row['thumb'] = ((empty($category['thumb']) ? '' : tomedia($category['thumb'])));
			if (0 < $row['deliveryer_id']) 
			{
				$row['deliveryer'] = pdo_get('tiny_wmall_deliveryer', array('uniacid' => $_W['uniacid'], 'id' => $row['deliveryer_id']));
			}
