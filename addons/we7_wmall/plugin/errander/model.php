...
function errander_delivery_times($idOrCategory) 
{
	...
	{
		$i = 0;
		while ($i <= $category['delivery_within_days']) 
		{
...
function errander_order_fetch($id) 
{
	...
	$order['category']['thumb'] = ((empty($order['category']['thumb']) ? '' : tomedia($order['category']['thumb'])));
...
function errander_order_status_notice($id, $status, $note = '') 
{
	...
	if (!(empty($order['openid']))) 
	{
		...
		if ($order_channel == 'wap') 
		{
			...
			{
				$miniprogram = array('appid' => $config_wxapp_basic['key'], 'pagepath' => 'pages/errander/orderDetail?id=' . $order['id']);
			}
			$status = $acc->sendTplNotice($order['openid'], $_W['we7_wmall']['config']['notice']['wechat']['public_tpl'], $send, $url, $miniprogram);
		}
		else 
		{
			$send = array( 'keyword1' => array('value' => '跑腿单', 'color' => '#ff510'), 'keyword2' => array('value' => $order['status_cn'], 'color' => '#ff510'), 'keyword3' => array('value' => $order['accept_username'], 'color' => '#ff510'), 'keyword4' => array('value' => $order['accept_mobile'], 'color' => '#ff510'), 'keyword5' => array('value' => $order['final_fee'], 'color' => '#ff510'), 'keyword6' => array('value' => date('Y-m-d H:i'), 'color' => '#ff510'), 'keyword7' => array('value' => $order['order_sn'], 'color' => '#ff510') );
			$public_tpl = $_W['we7_wmall']['config']['wxapp']['wxtemplate']['public_tpl'];
			$form_id = $order['data']['formId'];
			$form_type = 'formId';
			if (empty($form_id)) 
			{
				if (0 < $order['data']['prepay_times']) 
				{
					$form_type = 'prepayId';
			...
			if (!(empty($form_id))) 
			{
				...
				$status = $acc->sendTplNotice($order['openid'], $public_tpl, $send, 'pages/errander/orderDetail?id=' . $order['id'], $form_id);
			}
		}
		if (is_error($status)) 
		{
...
function errander_order_insert_refund_log($id, $type, $note = '') 
{
	...
	$notes = array( 'apply' => array('status' => 1, 'title' => '提交退款申请', 'note' => ''), 'handel' => array('status' => 2, 'title' => $_W['we7_wmall']['config']['mall']['title'] . '接受退款申请', 'note' => ''), 'success' => array('status' => 3, 'title' => '退款成功', 'note' => ''), 'fail' => array('status' => 4, 'title' => '退款失败', 'note' => '') );
	$title = $notes[$type]['title'];
	$note = (($note ? $note : $notes[$type]['note']));
...
function errander_order_deliveryer_notice($id, $type, $deliveryer_id = 0, $note = '') 
{
	...
	else if ($type == 'direct_transfer_refuse') 
	{
		$from_deliveryer = $note['from_deliveryer'];
...
function errander_order_status_update($id, $type, $extra = array()) 
{
	...
	if ($type == 'dispatch') 
	{
		...
		else if ($config['dispatch_mode'] == 2) 
		{
		}
		else 
		{
	...
	else if ($type == 'delivery_transfer') 
	{
		if ($order['status'] == 3) 
		{
	...
	else if ($type == 'direct_transfer_reply') 
	{
		if (empty($extra['deliveryer_id'])) 
		{
...
function errander_order_delivery_fee($idOrCategory, $extra) 
{
	...
	else 
	{
		...
		$fees['basic'] = array('title' => '起步价', 'note' => '含' . $category['start_km'] . '公里', 'fee' => $category['start_fee'], 'fee_cn' => '￥' . $category['start_fee']);
		$distance = 0;
		...
		if (($category['start_km'] < $distance) && (0 < $category['pre_km'])) 
		{
			...
			$fees[] = array('title' => '里程费', 'note' => $distance_over . '公里', 'fee' => $distance_over_fee, 'fee_cn' => '￥' . $distance_over_fee);
		}
		...
		if ($category['weight_fee_status'] == 1) 
		{
			...
			if ($category['weight_fee']['start_weight'] < $goods_weight) 
			{
				...
				$fees[] = array('title' => '重量费', 'note' => $goods_weight_over . '公斤', 'fee' => $goods_weight_over_fee, 'fee_cn' => '￥' . $goods_weight_over_fee);
			}
		}
		if (0 < $delivery_fee_predict_time) 
		{
			$message .= '特殊时间额外配送费' . $delivery_fee_predict_time . '元';
			$fees[] = array('title' => '特殊时间额外配送费', 'note' => '', 'fee' => $delivery_fee_predict_time, 'fee_cn' => '￥' . $delivery_fee_predict_time);
		}
	}
	if (0 < $tip) 
	{
		$fees[] = array('title' => '小费', 'note' => '', 'fee' => $tip, 'fee_cn' => '￥' . $tip);
	}
	$activityed = array();
	$discount_fee = 0;
	if (0 < $_W['member']['groupid']) 
	{
		...
		else if ($category['group_discount']['type'] == 2) 
		{
			$group_discount = $category['group_discount']['data'][$groupid];
			if ($group_discount['condition'] <= $delivery_fee) 
			{
		...
		if (0 < $discount_fee) 
		{
			$activityed[] = array('title' => $_W['member']['groupname'], 'note' => ($category['group_discount']['type'] == 1 ? '返' . $discount_fee . '元' : '打' . $group_discount['back'] . '折'), 'fee' => $discount_fee, 'fee_cn' => '-￥' . $discount_fee);
		}
	}
...
function errander_order_calculate_delivery_fee($idOrCategory, $extra, $is_calculate = 0) 
{
	...
	if ($rule_fee_type != 'fee') 
	{
		...
		if (!(empty($extra['acceptaddress']['location_x'])) && !(empty($extra['buyaddress']['location_y']))) 
		{
			...
			$distance_type = array('riding' => 2, 'driving' => 1, 'line' => __LINE__, 'walking' => 3);
	...
	$deliveryinfo = array();
	$i = 0;
	while ($i <= intval($rule_fee['fee_day_limit'])) 
	{
	...
	if ($rule_fee['extra_fee_time_status'] == 1) 
	{
		...
		{
			if ((strtotime($val['start_hour']) < strtotime($delivery_time)) && (strtotime($delivery_time) < strtotime($val['end_hour']))) 
			{
	...
	if (!(empty($activityed['redPacket']))) 
	{
		...
		$fees[] = array('title' => '红包', 'note' => '', 'fee' => -$activityed['redPacket']['discount'], 'fee_cn' => '-￥' . $activityed['redPacket']['discount']);
		...
		if (!(empty($extra['extra_fee']))) 
		{
			...
			foreach ($extra['extra_fee'] as $key => $val ) 
			{
				...
				foreach ($val as $v ) 
				{
					...
					$fees[] = array('title' => $rule_fee['extra_fee'][$key]['title'] . '-' . $extra_fee_name, 'note' => $extra_fee_name, 'fee' => $extra_item_fee, 'fee_cn' => '￥' . $extra_item_fee);
				}
			...
			foreach ($diypage['data']['fees']['extra_fee'] as $pindex => $item ) 
			{
				if ($item['status'] == 1) 
				{
					...
					foreach ($extra_fee_store[$pindex] as $val ) 
					{
						...
						$extra_fee += $extra_item_fee;
						$fees[] = array('title' => $rule_fee['extra_fee'][$pindex]['title'] . '-' . $extra_fee_name, 'note' => $extra_fee_name, 'fee' => $extra_item_fee, 'fee_cn' => '￥' . $extra_item_fee);
					}
					$extra_fees += $extra_fee;
				}
				else 
...
function errander_order_calculate_deliveryer_fee($order, $deliveryerOrid = 0) 
{
	...
	else if ($config_errander['deliveryer_fee_type'] == 3) 
	{
...
function get_errander_rule_fee($diypageorId) 
{
	...
	$distance_fee = $rule_fee_distance['fee'];
	$fees['basic'] = array( 'title' => '基础配送费', 'name' => 'basic', 'items' => array( array('note' => '每单', 'fee' => $distance_fee, 'fee_cn' => $distance_fee . '元') ) );
	if ($rule_fee_type != 'fee') 
	{
		$rule_fee_wrap = $rule_fee_distance[$rule_fee_type];
		if (!(empty($rule_fee_wrap['data']))) 
		...
		if ($rule_fee_type == 'distance') 
		{
			$fees['basic'] = array( 'title' => '基础配送费', 'name' => 'basic', 'items' => array( array('note' => $rule_fee_item['start_km'] . '公里内', 'fee' => $rule_fee_item['start_fee'], 'fee_cn' => $rule_fee_item['start_fee'] . '元') ) );
		...
		else if ($rule_fee_type == 'section') 
		{
			...
			foreach ($rule_fee_item as $row ) 
			{
				$fees['distance']['items'][] = array('note' => $row['start_km'] . '-' . $row['end_km'] . '公里', 'fee' => $row['fee'], 'fee_cn' => $row['fee'] . '元');
			}
	...
	if ($rule_fee['extra_fee_time_status'] == 1) 
	{
		...
		foreach ($rule_fee['extra_fee_time_data']['data'] as $val ) 
		{
			$fees['special_time']['items'][] = array('note' => $val['start_hour'] . '-' . $val['end_hour'], 'fee' => $val['fee'], 'fee_cn' => '+' . $val['fee'] . '元');
		}
	}
	...
	foreach ($rule_fee['extra_fee'] as $item ) 
	{
		if ($item['status'] == 1) 
		{
			...
			{
				$fees['extra_fee']['items'][] = array('note' => $item['title'] . '-' . $val['fee_name'], 'fee' => $val['fee'], 'fee_cn' => '+' . $val['fee'] . '元');
			}
		}
	}
