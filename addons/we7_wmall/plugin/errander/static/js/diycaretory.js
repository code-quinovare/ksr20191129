define(['jquery.ui', 'clockpicker'], function(ui, $) {
	...
	diy.initParts = function(){
		diy.getParts();
		var partGroup = {
			0: ['banner', 'basic', 'text', 'oneChoice', 'multipleChoices', 'line', 'blank', 'picture'],
		};
		...
		$("#parts nav").unbind('click').click(function(){
			...
			if($.inArray(id, ['text', 'oneChoice', 'multipleChoices']) != -1 && !$('#basic-extra').length) {
	...
	diy.initItems = function(selected) {
		...
		$.each(diy.items, function(itemid, item) {
			if(typeof(item.id) !== 'undefined') {
				...
				if($.inArray(item.id, ['text', 'oneChoice', 'multipleChoices']) != -1) {
