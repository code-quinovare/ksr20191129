...
function creditshop_order_update($orderOrId, $type, $extra = array()) 
{
	...
	else if ($type == 'handle') 
	{
		if ($order['status'] == 1) 
		{
			pdo_update('tiny_wmall_creditshop_order_new', array('status' => 2), array('id' => $order['id']));
			return;
		}
	}
	else if ($type == 'cancel') 
	{
		if (($order['is_pay'] == 0) && ($order['status'] == 1) && (0 < $order['use_credit1']) && ($order['use_credit1_status'] == 1) && (0 < $order['use_credit2'])) 
		{
