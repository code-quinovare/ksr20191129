...
function superRedpacket_grant_show() {
	...
	if(!empty($gift_redpacket)) {
		...
		$today = date('Ymd');
		$uid = $openid = '';
		if($gift_redpacket['starttime'] <= TIMESTAMP && $gift_redpacket['endtime'] > TIMESTAMP) {//活动时间
			...
			if($grant_object[$today]['success'] < $gift_params['everyday_nums']) {//每日数量
				$start_hour = strtotime($gift_params['start_hour']);
				$end_hour = strtotime($gift_params['end_hour']);
				if($start_hour <= TIMESTAMP && $end_hour > TIMESTAMP) {//发放时间
					if(!empty($_W['member']['uid'])) {//uid存在
						$uid = $_W['member']['uid'];
