	...
	if ($_W['ispost']) 
	{
		...
		foreach ($uids as $key => $uid ) 
		{
			...
			foreach ($superRedpacket['data']['redpackets'] as $redpacket ) 
			{
				...
				$params = array('title' => $redpacket['name'], 'activity_id' => $superRedpacket['id'], 'uid' => $uid, 'channel' => 'superRedpacket', 'type' => 'grant', 'discount' => $redpacket['discount'], 'condition' => $redpacket['condition'], 'grant_days_effect' => $redpacket['grant_days_effect'], 'days_limit' => $redpacket['use_days_limit'], 'is_show' => 0, 'scene' => $redpacket['scene']);
				$times_limit = array();
				if (!(empty($redpacket['times']))) 
				{
