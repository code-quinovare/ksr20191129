...
if ($op == 'post') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$insert = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'scene' => $_GPC['data']['page']['scene'], 'name' => $_GPC['data']['page']['name'], 'data' => base64_encode(json_encode($_GPC['data'])), 'agreement' => $agreement);
		if (!(empty($id))) 
		{
			$insert['updatetime'] = TIMESTAMP;
