...
if ($op == 'delivery') 
{
	...
	if ($_W['ispost']) 
	{
		...
		if ($delivery['delivery_fee_mode'] == 2) 
		{
			...
			$delivery['delivery_fee'] = array('start_fee' => trim($_GPC['start_fee']), 'start_km' => trim($_GPC['start_km']), 'pre_km_fee' => trim($_GPC['pre_km_fee']), 'distance_type' => intval($_GPC['distance_type']), 'calculate_distance_type' => intval($_GPC['calculate_distance_type']), 'max_fee' => intval($_GPC['max_fee']));
		}
		...
		$update = array('delivery_mode' => $delivery['delivery_mode'], 'delivery_fee_mode' => $delivery['delivery_fee_mode'], 'delivery_price' => $delivery['delivery_fee'], 'delivery_free_price' => $delivery['delivery_free_price'], 'delivery_times' => iserializer($times), 'delivery_areas' => iserializer($delivery_areas), 'delivery_extra' => iserializer($delivery['delivery_extra']));
		if ($delivery['delivery_fee_mode'] == 2) 
		{
