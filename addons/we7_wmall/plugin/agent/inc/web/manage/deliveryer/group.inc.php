...
{
	$_W["page"]["title"] = "配送员等级";
	$groups = pdo_fetchall("select * from" . tablename("tiny_wmall_deliveryer_groups") . "where uniacid = :uniacid and agentid = :agentid", array( ":uniacid" => $_W["uniacid"], ":agentid" => $_W["agentid"] ));
...
if( $op == "post" ) 
{
	...
	if( 0 < $id ) 
	{
		$group = pdo_get("tiny_wmall_deliveryer_groups", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
		$group["delivery_fee"] = iunserializer($group["delivery_fee"]);
	}
	if( $_W["ispost"] ) 
	{
		...
		$data = array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "title" => $title, "group_condition" => floatval($_GPC["group_condition"]), "delivery_fee" => iserializer($delivery_fee) );
		if( empty($group["id"]) ) 
		{
		...
		{
			pdo_update("tiny_wmall_deliveryer_groups", $data, array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
		}
...
{
	$id = intval($_GPC["id"]);
	pdo_delete("tiny_wmall_deliveryer_groups", array( "uniacid" => $_W["uniacid"], "agentid" => $_W["agentid"], "id" => $id ));
