	...
	public function parse() 
	{
		...
		if ($type == 'order') 
		{
			...
			if (!(empty($config_meituan))) 
			{
				...
				else if ($config_meituan_delivery['delivery_mode'] == 2) 
				{
			...
			$order['store_final_fee'] = -($platform_serve_fee + $order['plateform_delivery_fee']);
			$invoice = array();
		...
		else if ($type == 'orderShippingStatus') 
		{
			...
			else if ($message['shippingStatus'] == 40) 
			{
				order_status_update($order['id'], 'end', array('role' => 'meituan'));
			}
		}
