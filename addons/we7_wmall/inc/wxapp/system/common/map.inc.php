...
if ($ta == 'regeo_qq') 
{
	$query = array('output' => 'json', 'get_poi' => 1, 'key' => '2ECBZ-DXGLS-26MOI-6XMGC-QLTA6-SYFYL', 'location' => $_GPC['latitude'] . ',' . $_GPC['longitude']);
	...
	$data = array('address' => $result['formatted_addresses']['recommend'], 'location_x' => $result['location']['lat'], 'location_y' => $result['location']['lng'], 'latitude' => $result['location']['lat'], 'longitude' => $result['location']['lng'], 'locations' => $result['location']['lng'] . ', ' . $result['location']['lat']);
