...
if ($ta == 'index') 
{
	...
	foreach ($data as $da ) 
	{
		...
		$reserves[$da['table_cid']][] = array('time' => $da['time'], 'timeout' => $timeout);
	}
	...
	$result = array('categorys' => $categorys, 'reserves' => $reserves, 'days' => $days, 'year' => date('Y'));
...
else if ($ta == 'post') 
{
	...
	$order = array('order_type' => 4, 'price' => $cart['price'], 'total_fee' => $cart['price'] + $serve_fee, 'serve_fee' => $serve_fee, 'discount_fee' => $activityed['total'], 'activityed' => $activityed, 'coupon' => coupon_available_check($sid, $recordid, $cart['price']));
	$order['final_fee'] = $order['total_fee'] - $order['discount_fee'];
	$result = array('store' => $store, 'coupons' => $coupons, 'category' => $category, 'order' => $order, 'cart' => $cart, 'islegal' => true);
	imessage(error(0, $result), '', 'ajax');
}
else if ($ta == 'goodsIndex') 
{
