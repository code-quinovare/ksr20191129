...
if ($ta == 'account') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$data = array('uniacid' => $_W['uniacid'], 'mobile' => $mobile, 'title' => trim($_GPC['title']), 'openid' => $_W['openid_wechat'], 'openid_wxapp' => $_W['openid'], 'nickname' => $_W['member']['nickname'], 'avatar' => $_W['member']['avatar'], 'salt' => random(6), 'addtime' => TIMESTAMP);
		$data['password'] = md5(md5($data['salt'] . $password) . $data['salt']);
		pdo_insert('tiny_wmall_clerk', $data);
		$id = pdo_insertid();
		imessage(error(-1000, '继续完善商户信息'), '', 'ajax');
	}
	$result = array('mobile_verify_status' => $config_store['settle']['mobile_verify_status'], 'captcha' => imurl('system/common/captcha', array(), true));
...
if ($ta == 'store') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$data = array('uniacid' => $_W['uniacid'], 'agentid' => $_W['agentid'], 'title' => $title, 'address' => trim($_GPC['address']), 'telephone' => trim($_GPC['telephone']), 'content' => trim($_GPC['content']), 'status' => $config_store['settle']['audit_status'], 'business_hours' => iserializer(array( array('s' => '8:00', 'e' => '20:00') )), 'payment' => iserializer(array('wechat')), 'remind_time_limit' => 10, 'remind_reply' => iserializer(array('快递员狂奔在路上,请耐心等待')), 'addtype' => 2, 'addtime' => TIMESTAMP, 'delivery_mode' => $config_store['delivery']['delivery_mode'], 'delivery_fee_mode' => 1, 'delivery_price' => $config_store['delivery']['delivery_fee'], 'push_token' => random(32), 'self_audit_comment' => intval($config_store['settle']['self_audit_comment']));
