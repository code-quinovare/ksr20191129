...
if ($ta == 'list') 
{
	...
	$stores = pdo_fetchall('select id,title,logo,send_price,pack_price,delivery_fee_mode,delivery_price,delivery_areas,score,delivery_time,delivery_mode from' . tablename('tiny_wmall_store') . $condition, array(':uniacid' => $_W['uniacid'], ':agentid' => $_W['agentid']), 'id');
	if (!(empty($stores))) 
	{
