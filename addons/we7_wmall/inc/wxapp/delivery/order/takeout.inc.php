...
if ($ta == 'list') 
{
	...
	if ($status == 3) 
	{
		...
		else if ($_W['deliveryer']['perm_takeout'] == 2) 
		{
	...
	$orders = pdo_fetchall('SELECT id,order_plateform,serial_sn, addtime, is_pay, pay_type,order_type, status, username, mobile, address,distance, delivery_status,deliveryer_id,plateform_deliveryer_fee, delivery_type, delivery_fee, delivery_day, delivery_time, sid, num, final_fee, delivery_collect_type, transfer_delivery_status,location_x,location_y,note FROM ' . tablename('tiny_wmall_order') . $condition . $order_by . $limit, $params);
	if (!(empty($orders))) 
	{
		$stores_id = array();
	...
	$result = array('orders' => $orders, 'max_dispatching' => $config_takeout['order']['max_dispatching'], 'can_collect_order' => $can_collect_order, 'deliveryer' => $_W['deliveryer']);
...
if ($ta == 'status') 
{
	...
	$extra = array('deliveryer_id' => $deliveryer['id']);
...
if ($ta == 'detail') 
{
	...
	$result = array('order' => $order, 'deliveryer' => $_deliveryer);
	imessage(error(0, $result), '', 'ajax');
	return 1;
}
if ($ta == 'op') 
