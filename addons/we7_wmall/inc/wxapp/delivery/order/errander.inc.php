...
if ($ta == 'list') 
{
	...
	$result = array('orders' => $orders, 'can_collect_order' => $can_collect_order, 'deliveryer' => $_W['deliveryer'], 'verification_code' => ($config_errander['verification_code'] == 1 ? 1 : 0));
...
else if ($ta == 'detail') 
{
	...
	$result = array('order' => $order, 'deliveryer' => $_deliveryer, 'verification_code' => ($config_errander['verification_code'] == 1 ? 1 : 0));
	imessage(error(0, $result), '', 'ajax');
}
if ($ta == 'op') 
{
