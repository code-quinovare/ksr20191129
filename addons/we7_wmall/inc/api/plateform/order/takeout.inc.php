...
if ($ta == 'list') 
{
	...
	if (!(empty($orders))) 
	{
		...
		{
			$da['serial_sn'] = $da['id'] . '-' . $da['serial_sn'];
			...
			$da['deliverytime_cn'] = $da['delivery_day'] . ' ' . $da['delivery_time'];
		}
	}
...
if ($ta == 'detail') 
{
	...
	unset($order['data'], $order['invoice']);
	$order['deliverytime_cn'] = $order['delivery_day'] . ' ' . $order['delivery_time'];
...
if ($ta == 'status') 
{
	...
	if (in_array($type, array('notify_deliveryer_collect', 're_notify_deliveryer_collect', 'handle', 'notify_clerk_handle', 'end'))) 
	{
		...
		else if ($type == 're_notify_deliveryer_collect') 
		{
			$extra['force'] = 1;
	...
	if ($type == 'cancel') 
	{
		...
		$result = order_status_update($id, 'cancel', array('force_cancel' => 1, 'reason' => $reason, 'remark' => $remark, 'note' => $reason . ' ' . $remark));
		if (is_error($result)) 
		{
			message(ierror(-1, '处理编号为:' . $id . ' 的订单失败，具体原因：' . $result['message']), '', 'ajax');
		}
