...
if ($op == 'list') 
{
	...
	$orders = pdo_fetchall('SELECT id,serial_sn,delivery_collect_type,transfer_deliveryer_id,transfer_delivery_status,ordersn,order_type,order_plateform,distance,addtime, status, username, mobile, address, location_x, location_y, delivery_status, delivery_type, delivery_fee,plateform_deliveryer_fee,delivery_time,sid, num, final_fee, note FROM ' . tablename('tiny_wmall_order') . $condition, $params, 'id');
	$min = $max = 0;
	if (!(empty($orders))) 
	{
		$stores_id = array();
	...
	$data = array('list' => $orders, 'max_id' => $max, 'min_id' => $min, 'more' => $more, 'can_collect_order' => $can_collect_order, 'can_collect_order_cn' => ($can_collect_order == 1 ? '' : '当前调度模式不允许抢单,请等待管理员/系统派单'));
...
if ($op == 'detail') 
{
	...
	$order['delivery_assign_time_cn'] = $order['delivery_instore_time_cn'] = $order['delivery_success_time_cn'] = $order['delivery_takegoods_time_cn'] = array('day' => '未知', 'time' => '未知');
