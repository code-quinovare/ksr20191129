...
if ($ta == 'list') 
{
	...
	if (!(empty($stores))) 
	{
		...
		foreach ($stores as $key => &$da ) 
		{
			...
			$da['displayorder_order'] = (($da['displayorder'] + 1) * $da['is_in_business_hours_']);
			$da['sailed_order'] = (($da['sailed'] + 1) * $da['is_in_business_hours_']);
			$da['score_order'] = (($da['score'] + 1) * $da['is_in_business_hours_']);
			$da['click_order'] = (($da['click'] + 1) * $da['p']);
