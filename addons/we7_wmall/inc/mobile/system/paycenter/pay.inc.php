...
$params = array('module' => 'we7_wmall', 'ordersn' => $record['order_sn'], 'tid' => $record['order_sn'], 'user' => $_W['member']['uid'], 'openid' => $order['openid'], 'fee' => $record['fee'], 'title' => $title, 'order_type' => $type, 'sid' => $order['sid'], 'url_detail' => $routers[$type]['url_detail']);
$url_pay = $url_detail = '';
if ($type == 'takeout') 
{
...
if (is_h5app()) 
{
	$payinfo = array('mallName' => $_config['mall']['title'], 'money' => $log['card_fee'], 'ordersn' => $log['uniontid'], 'desc' => $title, 'attach' => $_W['uniacid'] . ':h5app');
}
else if (is_qianfan()) 
{
