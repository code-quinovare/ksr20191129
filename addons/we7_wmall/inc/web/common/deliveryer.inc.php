...
if ($op == 'list') 
{
	if (isset($_GPC['key'])) 
	{
		...
		$data = pdo_fetchall('SELECT * FROM ' . tablename('tiny_wmall_deliveryer') . ' where uniacid = :uniacid and ' . $type . ' = 1 and title like :key order by id desc', array(':uniacid' => $_W['uniacid'], ':key' => '%' . $key . '%'), 'id');
		if (!(empty($data))) 
		{
