...
if ($op == 'index') 
{
	...
	$psize = 15;
	$activitis = pdo_fetchall('select a.* from' . tablename('tiny_wmall_store_activity') . 'as a left join' . tablename('tiny_wmall_store') . 'as b on a.sid = b.id and b.id in (' . $stores_ids . ')' . $condition . ' ORDER BY b.id desc LIMIT ' . (($pindex - 1) * $psize) . ',' . $psize, $params);
...
if ($op == 'add') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$title = array();
		if (($type == 'discount') || ($type == 'cashGrant') || ($type == 'selfDelivery') || ($type == 'deliveryFeeDiscount')) 
		{
			$data = array();
