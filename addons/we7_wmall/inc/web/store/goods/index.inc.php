...
if ($ta == 'post') 
{
	...
	if ($_W['ispost']) 
	{
		$data = array('sid' => $sid, 'uniacid' => $_W['uniacid'], 'title' => trim($_GPC['title']), 'number' => trim($_GPC['number']), 'price' => floatval($_GPC['price']), 'old_price' => floatval($_GPC['old_price']), 'unitname' => trim($_GPC['unitname']), 'total' => intval($_GPC['total']), 'total_warning' => intval($_GPC['total_warning']), 'total_update_type' => intval($_GPC['total_update_type']), 'sailed' => intval($_GPC['sailed']), 'status' => intval($_GPC['status']), 'cid' => intval($_GPC['category']['parentid']), 'child_id' => intval($_GPC['category']['childid']), 'box_price' => floatval($_GPC['box_price']), 'thumb' => trim($_GPC['thumb']), 'label' => trim($_GPC['label']), 'displayorder' => intval($_GPC['displayorder']), 'content' => trim($_GPC['content']), 'description' => htmlspecialchars_decode($_GPC['description']), 'is_options' => intval($_GPC['is_options']), 'is_hot' => intval($_GPC['is_hot']), 'print_label' => intval($_GPC['print_label']), 'is_showtime' => intval($_GPC['is_showtime']));
		...
		if (!(empty($_GPC['is_showtime']))) 
		{
			...
			$data['start_time1'] = $data['end_time1'] = $data['start_time2'] = $data['end_time2'] = '';
	...
	if ($_W['ispost']) 
	{
		...
		if (!(empty($results))) 
		{
			...
			foreach ($results as &$result ) 
			{
				...
				if (empty($category)) 
				{
					$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'title' => $result['name'], 'status' => $result['isValid'], 'elemeId' => $result['id']);
					...
					if (!(empty($result['children']))) 
					{
						...
						{
							$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'title' => $val['name'], 'status' => $val['isValid'], 'elemeId' => $val['id'], 'parentid' => $parentid);
							pdo_insert('tiny_wmall_goods_category', $data);
							++$insert;
							$childs2parent[] = $val;
						}
				...
				{
					$data = array('title' => $result['name'], 'status' => $result['isValid']);
					...
					if (!(empty($result['children']))) 
					{
						foreach ($result['children'] as $val ) 
						{
							...
							{
								$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'title' => $val['name'], 'status' => $val['isValid'], 'elemeId' => $val['id'], 'parentid' => $category['id']);
								pdo_insert('tiny_wmall_goods_category', $data);
								++$insert;
							}
							else 
							{
								$data = array('title' => $val['name'], 'status' => $val['isValid'], 'parentid' => $category['id']);
...
if ($ta == 'eleme') 
{
	...
	if ($_W['ispost']) 
	{
		...
		{
			$data = array('uniacid' => $_W['uniacid'], 'sid' => $sid, 'cid' => (empty($classId['parentid']) ? $classId['id'] : $classId['parentid']), 'child_id' => (empty($classId['parentid']) ? 0 : $classId['id']), 'title' => $good['name'], 'unitname' => $good['unit'], 'thumb' => $good['imageUrl'], 'status' => $good['isValid'], 'price' => $good['specs'][0]['price'], 'total' => -1, 'elemeId' => $good['id']);
