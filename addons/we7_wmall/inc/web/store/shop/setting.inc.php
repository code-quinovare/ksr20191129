	...
	if ($_W['ispost']) 
	{
		...
		if ($item['delivery_mode'] == 1) 
		{
			...
			else if ($data['delivery_fee_mode'] == 2) 
			{
				...
				$data['auto_get_address'] = 1;
				$data['not_in_serve_radius'] = intval($_GPC['not_in_serve_radius']);
				$data['delivery_price'] = iserializer(array('start_fee' => trim($_GPC['start_fee']), 'start_km' => trim($_GPC['start_km']), 'pre_km_fee' => trim($_GPC['pre_km_fee']), 'calculate_distance_type' => intval($_GPC['calculate_distance_type']), 'distance_type' => intval($_GPC['distance_type']), 'max_fee' => intval($_GPC['max_fee'])));
			}
