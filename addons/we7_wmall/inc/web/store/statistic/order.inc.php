...
if ($ta == 'index') 
{
	...
	if ($_W['isajax']) 
	{
		...
		$chart = array( 'stat' => $stat, 'fields' => array('total_success_order', 'total_fee', 'store_final_fee', 'store_discount_fee', 'plateform_discount_fee', 'plateform_serve_fee', 'plateform_delivery_fee', 'total_cancel_order', 'total_cancel_fee'), 'titles' => array('有效订单量', '营业总额', '总收入', '商家补贴', '平台补贴', '平台服务费', '平台配送费', '无效订单量', '损失营业额') );
		$i = $starttime;
		while ($i <= $endtime) 
		{
			$chart['days'][] = $i;
