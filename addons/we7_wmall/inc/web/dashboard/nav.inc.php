...
if ($op == 'list') 
{
	...
	foreach ($navs as &$da ) 
	{
		...
		{
			$da['is_sys'] = 1;
			$da['link'] = imurl('wmall/home/search', array('cid' => $da['id'], 'order' => $_GPC['order'], 'dis' => $_GPC['dis']), true);
		}
...
if ($op == 'post') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$data = array('uniacid' => $_W['uniacid'], 'title' => $title, 'thumb' => trim($_GPC['thumb']), 'link' => trim($_GPC['link']), 'displayorder' => intval($_GPC['displayorder']), 'nav_status' => intval($_GPC['nav_status']), 'slide_status' => intval($_GPC['slide_status']), 'nav' => iserializer($nav), 'slide' => iserializer($slide), 'wxapp_link' => trim($_GPC['wxapp_link']));
		if (empty($_GPC['id'])) 
		{
