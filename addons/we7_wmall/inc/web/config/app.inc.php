...
$downurls = array( 'deliveryer' => array('ios' => MODULE_URL . 'resource/apps/' . $_W['uniacid'] . '/ios/deliveryman_1.0.apk', 'android' => MODULE_URL . 'resource/apps/' . $_W['uniacid'] . '/android/deliveryman_1.0.apk'), 'manager' => array('ios' => MODULE_URL . 'resource/apps/' . $_W['uniacid'] . '/ios/manager.apk', 'android' => MODULE_URL . 'resource/apps/' . $_W['uniacid'] . '/android/manager.apk') );
if ($op == 'deliveryer') 
{
	$_W['page']['title'] = '配送员app设置';
	if ($_W['ispost']) 
	{
		$data = array( 'serial_sn' => trim($_GPC['deliveryer']['serial_sn']), 'push_key' => trim($_GPC['deliveryer']['push_key']), 'push_secret' => trim($_GPC['deliveryer']['push_secret']), 'push_tags' => $_config['app']['deliveryer']['push_tags'], 'ios_build_type' => intval($_GPC['deliveryer']['ios_build_type']), 'android_version' => intval($_GPC['deliveryer']['android_version']), 'version' => array('ios' => trim($_GPC['deliveryer']['version']['ios']), 'android' => 1) );
...
if ($op == 'manager') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$data = array( 'serial_sn' => trim($_GPC['manager']['serial_sn']), 'push_key' => trim($_GPC['manager']['push_key']), 'push_secret' => trim($_GPC['manager']['push_secret']), 'ios_build_type' => intval($_GPC['manager']['ios_build_type']), 'version' => array('ios' => trim($_GPC['manager']['version']['ios']), 'android' => 1), 'phonic' => $files );
...
if ($op == 'customer') 
{
	...
	if ($_W['ispost']) 
	{
		...
		$data = array('serial_sn' => trim($_GPC['customer']['serial_sn']), 'appid' => trim($_GPC['customer']['appid']), 'key' => trim($_GPC['customer']['key']), 'login' => $login);
