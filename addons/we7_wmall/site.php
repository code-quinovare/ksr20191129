...
include IA_ROOT . '/addons/we7_wmall/version.json';
include 'defines.php';
include 'model.php';
class We7_wmallModuleSite extends WeModuleSite 
{
	...
	public function payResult($params) 
	{
		...
		$config = get_system_config();
		$_W['we7_wmall']['config'] = $config;
		if ((($params['result'] == 'success') && ($params['from'] == 'notify')) || (($params['from'] == 'return') && in_array($params['type'], array('delivery', 'credit')))) 
		{
			mload()->model('order');
			mload()->model('store');
			if (!(empty($record))) 
			...
			else if ($record['order_type'] == 'advertise') 
			{
				include WE7_WMALL_PLUGIN_PATH . 'advertise/model.php';
				advertise_trade_update($record['order_id']);
			}
			else if ($record['order_type'] == 'creditshop') 
			{
				include WE7_WMALL_PLUGIN_PATH . 'creditshop/model.php';
				creditshop_order_update($record['order_id'], 'pay', $params);
			}
			else if ($record['order_type'] == 'superredpacket_meal') 
			{
				include WE7_WMALL_PLUGIN_PATH . 'superRedpacket/model.php';
				superRedpacket_meal_order_order_update($record['order_id'], 'pay', $params);
			}
		}
		if ($params['from'] == 'return') 
		{
			...
			{
				$url = imurl('superRedpacket/meal/mealorder', array(), true);
			}
