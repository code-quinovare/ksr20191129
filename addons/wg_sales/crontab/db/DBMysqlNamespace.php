...
class Logger {
    ...
    public static function logError($message, $category ) {
        //file_put_contents(dirname(__FILE__) . '/../news_error.log',$category . '|' . date('Y-m-d H:i:s') . '|'. $message . "\n", FILE_APPEND);
    }

    public static function logWarn($message, $category ) {
        //file_put_contents(dirname(__FILE__) . '/../news_warm.log', $category . '|' .date('Y-m-d H:i:s') .'|'. $message . "\n", FILE_APPEND);
    }
}
