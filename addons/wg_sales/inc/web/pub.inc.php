...
class Wg_sales_Web_Pub extends SalesBaseController
{
	public $cate = array();
	public $size = 10;
	...
	function status()
	{
		...
		$article_id = $article['article_id'];
		if ($status == 1) {
			unset($article['id'], $article['uid'], $article['cate'], $article['status'], $article['money'], $article['category_id'], $article['article_id'], $article['update_time']);
		...
		$this->site->pubModule->update(['id' => $id], ['article_id' => $article_id, 'status' => $status, 'update_time' => time()]);
		echo json_encode(['code' => 0]);
		exit;
	}
}