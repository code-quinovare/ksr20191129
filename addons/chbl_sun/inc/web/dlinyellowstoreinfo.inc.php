...
if($info['imgs']){
        ...
        if(checksubmit('submit')){
        	...
        	$res = pdo_update('chbl_sun_yellowstore', $data, array('id' => $_GPC['id']));
        	if($res){
        		message('编辑成功',$this->createWebUrl2('dlinyellowstore',array()),'success');
        	}else{
        		message('编辑失败','','error');
        	}

        }
include $this->template('web/dlinyellowstoreinfo');