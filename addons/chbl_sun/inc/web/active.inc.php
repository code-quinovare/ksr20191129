...
$info=pdo_get('chbl_sun_active',array('uniacid' => $_W['uniacid']));
...
$sql = 'SELECT COUNT(*) FROM ' . tablename('chbl_sun_active') .$condition ;

$total = pdo_fetchcolumn($sql, $params);

if (!empty($total)) {
    $sql = 'SELECT * FROM  ' . tablename('chbl_sun_active') .$condition.' ORDER BY  `sort`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
...
if($operation == 'done'){
    $id = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id FROM " . tablename('chbl_sun_active') . " WHERE id = :id", array(':id' => $id));
    if (empty($row)) {
    ...
    if($status == 1)
    {
        ...
        $active = pdo_fetch("SELECT id FROM " . tablename('chbl_sun_active') .$condition , $params);

        pdo_update('chbl_sun_active', array('status'=>$status), array('id' => $id));
...
if($operation == 'delete'){
    ...
    $res = pdo_delete('chbl_sun_active',array('id'=>$id));
    if($res){
