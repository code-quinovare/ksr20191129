...
//黄页数量
$hytotal=pdo_get('chbl_sun_yellowstore', array('uniacid'=>$_W['uniacid'],'state'=>2), array('count(id) as count'));
//资讯数量
...
//帖子入驻加置顶
$sql9=" select sum(money) as tzmoney from".tablename('chbl_sun_tzpaylog')." where uniacid={$_W['uniacid']} and  time like '%{$time}%' ";
$tzmoney=pdo_fetch($sql9); 
//拼车发布的钱
$sql10=" select sum(money) as pcmoney from".tablename('chbl_sun_carpaylog')." where uniacid={$_W['uniacid']} and  time like '%{$time}%' ";
$pcmoney=pdo_fetch($sql10); 
//114入驻的钱
$sql11=" select sum(money) as hymoney from".tablename('chbl_sun_yellowpaylog')." where uniacid={$_W['uniacid']} and  time like '%{$time}%' ";
