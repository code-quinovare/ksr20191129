...
$info=pdo_get('chbl_sun_bargain',array('uniacid' => $_W['uniacid']));
...
$sql = 'SELECT COUNT(*) FROM ' . tablename('chbl_sun_bargain') .$condition ;

$total = pdo_fetchcolumn($sql, $params);

if (!empty($total)) {
    $sql = 'SELECT * FROM  ' . tablename('chbl_sun_bargain') .$condition.' ORDER BY  `selftime`  DESC LIMIT ' . ($pindex - 1) * $psize . ',' . $psize;
...
if($operation == 'done'){
    $id = intval($_GPC['id']);
    $row = pdo_fetch("SELECT id FROM " . tablename('chbl_sun_bargain') . " WHERE id = :id", array(':id' => $id));
    if (empty($row)) {
    ...
    if($status == 1)
    {
        ...
        $active = pdo_fetch("SELECT id FROM " . tablename('chbl_sun_bargain') .$condition , $params);

        pdo_update('chbl_sun_bargain', array('status'=>$status), array('id' => $id));
...
if($operation == 'delete'){
    ...
    $res = pdo_delete('chbl_sun_bargain',array('id'=>$id));
    if($res){
