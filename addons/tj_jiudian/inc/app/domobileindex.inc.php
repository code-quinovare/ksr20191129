...
if($action=="")
{
    if($_W['ispost']){
        ...
        // 房间类型
       $rooms = $_GPC['rooms'];
       $_SESSION['latitude'] = $_GPC['latitude']; // 纬度
       $_SESSION['longitude'] = $_GPC['longitude']; //经度

       // file_put_contents("./log.txt",date('Y-m-d H:i:s').PHP_EOL.json_encode($_SESSION).PHP_EOL,FILE_APPEND);
       if ($rooms == 2) {
           $time = $_GPC['time'];
            $_SESSION['hour'] = $time;
           $_SESSION['lidian'] = date('Y-m-d H:i',strtotime('+'.$time.' hour',strtotime($_SESSION['ruzhu'])));
       }else if($rooms == 3){
            $day = date('Y-m-d',strtotime('+1 day',strtotime($_SESSION['ruzhu'])));
             $_SESSION['lidian'] = $day.' 11:30';
       }else{
            $_SESSION['lidian'] = $_GPC['lidian'];
       }
        message("",$this->createMobileUrl("Index",array("action"=>"list",'rooms'=>$_GPC['rooms'],'time'=>$time,'latitude'=>$_GPC['latitude'],'longitude'=>$_GPC['longitude'])));

        }


    // 推荐房源
    $remhouse = pdo_fetchall("SELECT * FROM ".tablename("tj_jiudian_house")." WHERE is_rem=1 AND acid = ".$_W['account']['acid'] . " order by rank desc");

    $zk = pdo_get('ewei_shop_member_level',array('id'=>5),array('discount'));
       
    unset($_SESSION['ruzhu']);
    unset($_SESSION['hour']);
    unset($_SESSION['lidian']);
...
if ($action == 'address') {
   ...
   $ak = "AB8650f063ed5cb61a3073ad56608cb7";
   $weizhi =  getAddressComponent($ak,$longitude, $latitude);

    echo $weizhi['result']['sematic_description']; 
}

if ($action == 'addresslist') {
    $tmp = $_GPC['address'];
    $rooms = $_GPC['rooms'];
    $address = $tmp['city'].$tmp['district'].$tmp['business'];
    $data = getJingWei($address);
    /*$longitude = $data['lng'];
    $latitude = $data['lat'];*/
    $url = $this->createMobileUrl("Index",array("action"=>"list",'rooms'=>$rooms,"longitude"=>$data['lng'],"latitude"=>$data['lat'],zuobiaotype=>1,"address"=>$address));
    // json_encode($address);
    echo $url;
}
...
if($action == "list")
{
    ...
    $longitude =  $_GPC['longitude'] ;
    $latitude =  $_GPC['latitude'];
    $zuobiaotype = $_GPC['zuobiaotype'];
   if ($zuobiaotype) {
       $zuobiao = array('longitude'=>$longitude,'latitude'=>$latitude);
       $weizhi['result']['sematic_description'] = $_GPC['address'];
   }else{
        $ak = "AB8650f063ed5cb61a3073ad56608cb7";
        $weizhi =  getAddressComponent($ak,$longitude, $latitude);
   ...
   $member = pdo_get('ewei_shop_member',array('uniacid'=>$_W['account']['acid'],'openid'=>$_W['openid']),array('level'));



    if ($member['level'] == 5) {
       $zk = pdo_get('ewei_shop_member_level',array('id'=>$member['level']),array('discount'));
    ...
    foreach($house as $k=>$v){
        ...
        $house[$k]['distance'] = getDistance($zuobiao['longitude'], $zuobiao['latitude'], $v['jingdu'], $v['weidu']);
...
if($action=='show'){
    ...
    // 评论
    $pingjia=pdo_fetchall("SELECT a.*,b.*,c.title,c.ruzhu FROM ".tablename("tj_jiudian_pinglun")." AS a LEFT JOIN ".tablename('tj_jiudian_huifu')." AS b on a.id=b.pinglun_id LEFT JOIN ".tablename('tj_jiudian_order')." AS c on a.oid=c.id WHERE c.tid = ".$_GPC['id']." AND a.acid = ".$_W['account']['acid'] . " order by time desc LIMIT 0,8 ");
    ...
    $member = pdo_get('ewei_shop_member',array('uniacid'=>$_W['account']['acid'],'openid'=>$_W['openid']),array('level'));
       ...
       $zk = pdo_get('ewei_shop_member_level',array('id'=>5),array('discount'));
       $res['stuprice'] = $res['price']*($zk['discount']/10);
...
if($action == "order_buy")
{
    ...
    $member = pdo_get('ewei_shop_member',array('openid'=>$_W['openid']),array('mobile','diymemberdata','uid'));
    // echo mobileUrl('member/info');
    if ($member['mobile']=='' || $member['diymemberdata']=='') {
        message('请绑定手机完善资料', mobileUrl('member/info'), 'error');
    }
    $deposit = pdo_get('tj_jiudian_deposit',array('uid'=>$member['uid']),array('money','status'));
    if ($deposit['status']!=1) {
         message('请先充值押金', mobileUrl('member/recharge'), 'error');
    }
...
if($action=='pinglun'){
    $id = $_GPC['id'];
    $pingjia=pdo_fetchall("SELECT a.*,b.*,c.title,c.ruzhu FROM ".tablename("tj_jiudian_pinglun")." AS a LEFT JOIN ".tablename('tj_jiudian_huifu')." AS b on a.id=b.pinglun_id LEFT JOIN ".tablename('tj_jiudian_order')." AS c on a.oid=c.id WHERE c.tid = ".$id." AND a.acid = ".$_W['account']['acid'] . " order by time desc LIMIT 0,8 ");
    foreach ($pingjia as $k => $v) {
        $pingjia[$k]['images']=unserialize($pingjia[$k]['images']);
    }
