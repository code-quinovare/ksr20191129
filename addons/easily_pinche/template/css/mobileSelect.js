...
(function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			...
			if(config.selected){
				_this.initPosition.push(Number(config.selected))  
			}
			//定位 初始位置
			...
			_this.cancelBtn.addEventListener('click',function(){
				...
				$(".mobileSelect-color .icon-sanjiao").removeClass("on");
		    });
		    ...
		    _this.ensureBtn.addEventListener('click',function(){
		    	...
		    	if(_this.triggerDisplayData){
		    		var arrSelf = tempValue.split(" ");
		    		var numSelf = arrSelf.length - 1;
		    		if($(".es-filter").length>0){
			    		_this.trigger.getElementsByTagName('span')[0].innerHTML = arrSelf[numSelf];
			    		$(".mobileSelect-color .icon-sanjiao").removeClass("on");
		    		}else{
		    			_this.trigger.innerHTML = arrSelf[numSelf];
		    		}
		    	}
		...
		checkDataType: function(){
			...
			if(_this.wheelsData[0].data && typeof(_this.wheelsData[0].data[0])=='object'){
				_this.jsonType = true;
			}
		},
